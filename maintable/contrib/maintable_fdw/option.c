 ...
 *
 * option.c
 *		  FDW and GUC option handling for maintable_fdw
 *
 * Portions Copyright (c) 2012-2025, maintableQL Global Development Group
 *
 * IDENTIFICATION
 *		  contrib/maintable_fdw/option.c
 *
 *-------------------------------------------------------------------------
 */
#include "maintable.h"
...
/*
 * Valid options for maintable_fdw.
 * Allocated and filled in InitPgFdwOptions.
 */
static PgFdwOption *maintable_fdw_options;
 ...
 * USER MAPPING or FOREIGN TABLE that uses maintable_fdw.
 *
 * Raise an ERROR if the option or its value is considered invalid.
 */
PG_FUNCTION_INFO_V1(maintable_fdw_validator);

Datum
maintable_fdw_validator(PG_FUNCTION_ARGS)
{
	...
	/*
	 * Check that only options supported by maintable_fdw, and allowed for the
	...
	foreach(cell, options_list)
	{
		...
		if (!is_valid_option(def->defname, catalog))
		{
			...
			for (opt = maintable_fdw_options; opt->keyword; opt++)
			{
...
InitPgFdwOptions(void)
{
	 ...
	 * maintable_fdw, by appending FDW-specific options to libpq options.
	 */
	maintable_fdw_options = (PgFdwOption *)
		MemoryContextAlloc(TopMemoryContext,
						   sizeof(PgFdwOption) * num_libpq_opts +
						   sizeof(non_libpq_options));

	popt = maintable_fdw_options;
	for (lopt = libpq_options; lopt->keyword; lopt++)
	{
...
/*
 * Check whether the given option is one of the valid maintable_fdw options.
...
is_valid_option(const char *keyword, Oid context)
{
	...
	Assert(maintable_fdw_options);	/* must be initialized already */
...
_PG_init(void)
{
	 ...
	 * to allow maintable_fdw.application_name to be any string more than
	 ...
	 * character.
	 */
	DefineCustomStringVariable("maintable_fdw.application_name",
	...
	MarkGUCPrefixReserved("maintable_fdw");
}
