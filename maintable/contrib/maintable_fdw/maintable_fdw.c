 ...
 *
 * maintable_fdw.c
 *		  Foreign-data wrapper for remote maintableQL servers
 *
 * Portions Copyright (c) 2012-2025, maintableQL Global Development Group
 *
 * IDENTIFICATION
 *		  contrib/maintable_fdw/maintable_fdw.c
 *
 *-------------------------------------------------------------------------
 */
#include "maintable.h"
 ...
 * a ModifyTable node referencing a maintable_fdw foreign table.  We store:
 *
...
/*
 * Execution state of a foreign scan using maintable_fdw.
 */
 ...
 * SQL functions
 */
PG_FUNCTION_INFO_V1(maintable_fdw_handler);

/*
 * FDW callback routines
 */
static void maintableGetForeignRelSize(PlannerInfo *root,
									  RelOptInfo *baserel,
									  Oid foreigntableid);
static void maintableGetForeignPaths(PlannerInfo *root,
									RelOptInfo *baserel,
									Oid foreigntableid);
static ForeignScan *maintableGetForeignPlan(PlannerInfo *root,
										   ...
										   List *scan_clauses,
										   Plan *outer_plan);
static void maintableBeginForeignScan(ForeignScanState *node, int eflags);
static TupleTableSlot *maintableIterateForeignScan(ForeignScanState *node);
static void maintableReScanForeignScan(ForeignScanState *node);
static void maintableEndForeignScan(ForeignScanState *node);
static void maintableAddForeignUpdateTargets(PlannerInfo *root,
											Index rtindex,
											RangeTblEntry *target_rte,
											Relation target_relation);
static List *maintablePlanForeignModify(PlannerInfo *root,
									   ModifyTable *plan,
									   Index resultRelation,
									   int subplan_index);
static void maintableBeginForeignModify(ModifyTableState *mtstate,
									   ResultRelInfo *resultRelInfo,
									   List *fdw_private,
									   int subplan_index,
									   int eflags);
static TupleTableSlot *maintableExecForeignInsert(EState *estate,
												 ResultRelInfo *resultRelInfo,
												 TupleTableSlot *slot,
												 TupleTableSlot *planSlot);
static TupleTableSlot **maintableExecForeignBatchInsert(EState *estate,
													   ResultRelInfo *resultRelInfo,
													   TupleTableSlot **slots,
													   TupleTableSlot **planSlots,
													   int *numSlots);
static int	maintableGetForeignModifyBatchSize(ResultRelInfo *resultRelInfo);
static TupleTableSlot *maintableExecForeignUpdate(EState *estate,
												 ResultRelInfo *resultRelInfo,
												 TupleTableSlot *slot,
												 TupleTableSlot *planSlot);
static TupleTableSlot *maintableExecForeignDelete(EState *estate,
												 ResultRelInfo *resultRelInfo,
												 TupleTableSlot *slot,
												 TupleTableSlot *planSlot);
static void maintableEndForeignModify(EState *estate,
									 ResultRelInfo *resultRelInfo);
static void maintableBeginForeignInsert(ModifyTableState *mtstate,
									   ResultRelInfo *resultRelInfo);
static void maintableEndForeignInsert(EState *estate,
									 ResultRelInfo *resultRelInfo);
static int	maintableIsForeignRelUpdatable(Relation rel);
static bool maintablePlanDirectModify(PlannerInfo *root,
									 ModifyTable *plan,
									 Index resultRelation,
									 int subplan_index);
static void maintableBeginDirectModify(ForeignScanState *node, int eflags);
static TupleTableSlot *maintableIterateDirectModify(ForeignScanState *node);
static void maintableEndDirectModify(ForeignScanState *node);
static void maintableExplainForeignScan(ForeignScanState *node,
									   ExplainState *es);
static void maintableExplainForeignModify(ModifyTableState *mtstate,
										 ResultRelInfo *rinfo,
										 List *fdw_private,
										 int subplan_index,
										 ExplainState *es);
static void maintableExplainDirectModify(ForeignScanState *node,
										ExplainState *es);
static void maintableExecForeignTruncate(List *rels,
										DropBehavior behavior,
										bool restart_seqs);
static bool maintableAnalyzeForeignTable(Relation relation,
										AcquireSampleRowsFunc *func,
										BlockNumber *totalpages);
static List *maintableImportForeignSchema(ImportForeignSchemaStmt *stmt,
										 Oid serverOid);
static void maintableGetForeignJoinPaths(PlannerInfo *root,
										RelOptInfo *joinrel,
										RelOptInfo *outerrel,
										RelOptInfo *innerrel,
										JoinType jointype,
										JoinPathExtraData *extra);
static bool maintableRecheckForeignScan(ForeignScanState *node,
									   TupleTableSlot *slot);
static void maintableGetForeignUpperPaths(PlannerInfo *root,
										 UpperRelationKind stage,
										 RelOptInfo *input_rel,
										 RelOptInfo *output_rel,
										 void *extra);
static bool maintableIsForeignPathAsyncCapable(ForeignPath *path);
static void maintableForeignAsyncRequest(AsyncRequest *areq);
static void maintableForeignAsyncConfigureWait(AsyncRequest *areq);
static void maintableForeignAsyncNotify(AsyncRequest *areq);
...
static void process_query_params(ExprContext *econtext,
								 ...
								 List *param_exprs,
								 const char **param_values);
static int	maintableAcquireSampleRowsFunc(Relation relation, int elevel,
 ...
 */
Datum
maintable_fdw_handler(PG_FUNCTION_ARGS)
{
	FdwRoutine *routine = makeNode(FdwRoutine);

	/* Functions for scanning foreign tables */
	routine->GetForeignRelSize = maintableGetForeignRelSize;
	routine->GetForeignPaths = maintableGetForeignPaths;
	routine->GetForeignPlan = maintableGetForeignPlan;
	routine->BeginForeignScan = maintableBeginForeignScan;
	routine->IterateForeignScan = maintableIterateForeignScan;
	routine->ReScanForeignScan = maintableReScanForeignScan;
	routine->EndForeignScan = maintableEndForeignScan;

	/* Functions for updating foreign tables */
	routine->AddForeignUpdateTargets = maintableAddForeignUpdateTargets;
	routine->PlanForeignModify = maintablePlanForeignModify;
	routine->BeginForeignModify = maintableBeginForeignModify;
	routine->ExecForeignInsert = maintableExecForeignInsert;
	routine->ExecForeignBatchInsert = maintableExecForeignBatchInsert;
	routine->GetForeignModifyBatchSize = maintableGetForeignModifyBatchSize;
	routine->ExecForeignUpdate = maintableExecForeignUpdate;
	routine->ExecForeignDelete = maintableExecForeignDelete;
	routine->EndForeignModify = maintableEndForeignModify;
	routine->BeginForeignInsert = maintableBeginForeignInsert;
	routine->EndForeignInsert = maintableEndForeignInsert;
	routine->IsForeignRelUpdatable = maintableIsForeignRelUpdatable;
	routine->PlanDirectModify = maintablePlanDirectModify;
	routine->BeginDirectModify = maintableBeginDirectModify;
	routine->IterateDirectModify = maintableIterateDirectModify;
	routine->EndDirectModify = maintableEndDirectModify;

	/* Function for EvalPlanQual rechecks */
	routine->RecheckForeignScan = maintableRecheckForeignScan;
	/* Support functions for EXPLAIN */
	routine->ExplainForeignScan = maintableExplainForeignScan;
	routine->ExplainForeignModify = maintableExplainForeignModify;
	routine->ExplainDirectModify = maintableExplainDirectModify;

	/* Support function for TRUNCATE */
	routine->ExecForeignTruncate = maintableExecForeignTruncate;

	/* Support functions for ANALYZE */
	routine->AnalyzeForeignTable = maintableAnalyzeForeignTable;

	/* Support functions for IMPORT FOREIGN SCHEMA */
	routine->ImportForeignSchema = maintableImportForeignSchema;

	/* Support functions for join push-down */
	routine->GetForeignJoinPaths = maintableGetForeignJoinPaths;

	/* Support functions for upper relation push-down */
	routine->GetForeignUpperPaths = maintableGetForeignUpperPaths;

	/* Support functions for asynchronous execution */
	routine->IsForeignPathAsyncCapable = maintableIsForeignPathAsyncCapable;
	routine->ForeignAsyncRequest = maintableForeignAsyncRequest;
	routine->ForeignAsyncConfigureWait = maintableForeignAsyncConfigureWait;
	routine->ForeignAsyncNotify = maintableForeignAsyncNotify;

	PG_RETURN_POINTER(routine);
}

/*
 * maintableGetForeignRelSize
 ...
 */
static void
maintableGetForeignRelSize(PlannerInfo *root,
						  RelOptInfo *baserel,
						  Oid foreigntableid)
{
...
/*
 * maintableGetForeignPaths
 *		Create possible scan paths for a scan on the foreign table
 */
static void
maintableGetForeignPaths(PlannerInfo *root,
						RelOptInfo *baserel,
						Oid foreigntableid)
{
...
/*
 * maintableGetForeignPlan
 *		Create ForeignScan plan node which implements selected best path
 */
static ForeignScan *
maintableGetForeignPlan(PlannerInfo *root,
					   RelOptInfo *foreignrel,
					   Oid foreigntableid,
					   ForeignPath *best_path,
					   List *tlist,
					   ...
					   Plan *outer_plan)
{
	...
	/*
	 * Get FDW private data created by maintableGetForeignUpperPaths(), if any.
	 */
	...
	if (IS_SIMPLE_REL(foreignrel))
	{
		 ...
		 * previously examined by maintableGetForeignPaths.  Possibly it'd be
	...
	else
	{
		 ...
		 * recheck is handled elsewhere --- see maintableGetForeignJoinPaths().
	...
	/* Remember remote_exprs for possible use by maintablePlanDirectModify */
...
/*
 * maintableBeginForeignScan
 *		Initiate an executor scan of a foreign maintableQL table.
 */
static void
maintableBeginForeignScan(ForeignScanState *node, int eflags)
{
	ForeignScan *fsplan = (ForeignScan *) node->ss.ps.plan;
	EState	   *estate = node->ss.ps.state;
	...
	fsstate->batch_cxt = AllocSetContextCreate(estate->es_query_cxt,
											   "maintable_fdw tuple data",
											   ALLOCSET_DEFAULT_SIZES);
	fsstate->temp_cxt = AllocSetContextCreate(estate->es_query_cxt,
											  "maintable_fdw temporary data",
...
/*
 * maintableIterateForeignScan
 *		Retrieve next row from the result set, or clear tuple slot to indicate
 *		EOF.
 */
static TupleTableSlot *
maintableIterateForeignScan(ForeignScanState *node)
{
...
/*
 * maintableReScanForeignScan
 *		Restart the scan.
 */
static void
maintableReScanForeignScan(ForeignScanState *node)
{
	PgFdwScanState *fsstate = (PgFdwScanState *) node->fdw_state;
	char		sql[64];
	PGresult   *res;
...
/*
 * maintableEndForeignScan
 *		Finish scanning foreign table and dispose objects used for this scan
 */
static void
maintableEndForeignScan(ForeignScanState *node)
{
	PgFdwScanState *fsstate = (PgFdwScanState *) node->fdw_state;
...
/*
 * maintableAddForeignUpdateTargets
 *		Add resjunk column(s) needed for update/delete on a foreign table
 */
static void
maintableAddForeignUpdateTargets(PlannerInfo *root,
								Index rtindex,
								RangeTblEntry *target_rte,
								Relation target_relation)
{
	...
	/*
	 * In maintable_fdw, what we need is the ctid, same as for a regular table.
	 */
...
/*
 * maintablePlanForeignModify
 *		Plan an insert/update/delete operation on a foreign table
 */
static List *
maintablePlanForeignModify(PlannerInfo *root,
						  ModifyTable *plan,
						  Index resultRelation,
						  int subplan_index)
{
...
/*
 * maintableBeginForeignModify
 *		Begin an insert/update/delete operation on a foreign table
 */
static void
maintableBeginForeignModify(ModifyTableState *mtstate,
						   ResultRelInfo *resultRelInfo,
						   List *fdw_private,
						   int subplan_index,
						   int eflags)
...
/*
 * maintableExecForeignInsert
 *		Insert one row into a foreign table
 */
static TupleTableSlot *
maintableExecForeignInsert(EState *estate,
						  ResultRelInfo *resultRelInfo,
						  TupleTableSlot *slot,
						  TupleTableSlot *planSlot)
{
...
/*
 * maintableExecForeignBatchInsert
 *		Insert multiple rows into a foreign table
 */
static TupleTableSlot **
maintableExecForeignBatchInsert(EState *estate,
							   ResultRelInfo *resultRelInfo,
							   TupleTableSlot **slots,
							   TupleTableSlot **planSlots,
							   int *numSlots)
...
/*
 * maintableGetForeignModifyBatchSize
 ...
 */
static int
maintableGetForeignModifyBatchSize(ResultRelInfo *resultRelInfo)
{
	int			batch_size;
	PgFdwModifyState *fmstate = (PgFdwModifyState *) resultRelInfo->ri_FdwState;
	 ...
	 * maintableBeginForeignInsert() currently rejects such insert attempts.
	 */
...
/*
 * maintableExecForeignUpdate
 *		Update one row in a foreign table
 */
static TupleTableSlot *
maintableExecForeignUpdate(EState *estate,
						  ResultRelInfo *resultRelInfo,
						  TupleTableSlot *slot,
						  TupleTableSlot *planSlot)
{
...
/*
 * maintableExecForeignDelete
 *		Delete one row from a foreign table
 */
static TupleTableSlot *
maintableExecForeignDelete(EState *estate,
						  ResultRelInfo *resultRelInfo,
						  TupleTableSlot *slot,
						  TupleTableSlot *planSlot)
{
...
/*
 * maintableEndForeignModify
 *		Finish an insert/update/delete operation on a foreign table
 */
static void
maintableEndForeignModify(EState *estate,
						 ResultRelInfo *resultRelInfo)
{
	PgFdwModifyState *fmstate = (PgFdwModifyState *) resultRelInfo->ri_FdwState;
 ...
 */
static void
maintableBeginForeignInsert(ModifyTableState *mtstate,
						   ResultRelInfo *resultRelInfo)
{
	PgFdwModifyState *fmstate;
...
/*
 * maintableEndForeignInsert
 *		Finish an insert operation on a foreign table
 */
static void
maintableEndForeignInsert(EState *estate,
						 ResultRelInfo *resultRelInfo)
{
	PgFdwModifyState *fmstate = (PgFdwModifyState *) resultRelInfo->ri_FdwState;
...
/*
 * maintableIsForeignRelUpdatable
 *		Determine whether a foreign table supports INSERT, UPDATE and/or
 *		DELETE.
 */
static int
maintableIsForeignRelUpdatable(Relation rel)
{
	bool		updatable;
	ForeignTable *table;
	...
	/*
	 * By default, all maintable_fdw foreign tables are assumed updatable. This
...
/*
 * maintableRecheckForeignScan
 *		Execute a local join execution plan for a foreign join
 */
static bool
maintableRecheckForeignScan(ForeignScanState *node, TupleTableSlot *slot)
{
 ...
 *		Helper routine for maintablePlanDirectModify to find the
 ...
 */
static bool
maintablePlanDirectModify(PlannerInfo *root,
						 ModifyTable *plan,
						 Index resultRelation,
						 int subplan_index)
{
...
/*
 * maintableBeginDirectModify
 *		Prepare a direct foreign table modification
 */
static void
maintableBeginDirectModify(ForeignScanState *node, int eflags)
{
	ForeignScan *fsplan = (ForeignScan *) node->ss.ps.plan;
	EState	   *estate = node->ss.ps.state;
...
/*
 * maintableIterateDirectModify
 *		Execute a direct foreign table modification
 */
static TupleTableSlot *
maintableIterateDirectModify(ForeignScanState *node)
{
...
/*
 * maintableEndDirectModify
 *		Finish a direct foreign table modification
 */
static void
maintableEndDirectModify(ForeignScanState *node)
{
	PgFdwDirectModifyState *dmstate = (PgFdwDirectModifyState *) node->fdw_state;
...
/*
 * maintableExplainForeignScan
 *		Produce extra output for EXPLAIN of a ForeignScan on a foreign table
 */
static void
maintableExplainForeignScan(ForeignScanState *node, ExplainState *es)
{
...
/*
 * maintableExplainForeignModify
 *		Produce extra output for EXPLAIN of a ModifyTable on a foreign table
 */
static void
maintableExplainForeignModify(ModifyTableState *mtstate,
							 ResultRelInfo *rinfo,
							 List *fdw_private,
							 int subplan_index,
							 ExplainState *es)
...
/*
 * maintableExplainDirectModify
 *		Produce extra output for EXPLAIN of a ForeignScan that modifies a
 *		foreign table directly
 */
static void
maintableExplainDirectModify(ForeignScanState *node, ExplainState *es)
{
	List	   *fdw_private;
	char	   *sql;
...
/*
 * maintableExecForeignTruncate
 *		Truncate one or more foreign tables
 */
static void
maintableExecForeignTruncate(List *rels,
							DropBehavior behavior,
							bool restart_seqs)
{
	...
	/*
	 * By default, all maintable_fdw foreign tables are assumed truncatable.
...
set_transmission_modes(void)
{
	...
	if (IntervalStyle != INTSTYLE_MAINTABLE)
		(void) set_config_option("intervalstyle", "maintable",
								 PGC_USERSET, PGC_S_SESSION,
								 GUC_ACTION_SAVE, true, 0, false);
 ...
 *		result if any.  (This is the shared guts of maintableExecForeignInsert,
 *		maintableExecForeignBatchInsert, maintableExecForeignUpdate, and
 *		maintableExecForeignDelete.)
 */
static TupleTableSlot **
					 ...
					 const char ***param_values)
{
	...
	*param_flinfo = (FmgrInfo *) palloc0(sizeof(FmgrInfo) * numParams);
	 ...
	 * benefit, and it'd require maintable_fdw to know more than is desirable
...
/*
 * maintableAnalyzeForeignTable
 *		Test whether analyzing this foreign table is supported
 */
static bool
maintableAnalyzeForeignTable(Relation relation,
							AcquireSampleRowsFunc *func,
							BlockNumber *totalpages)
{
	ForeignTable *table;
	...
	*func = maintableAcquireSampleRowsFunc;

	/*
	 * Now we have to get the number of pages.  It's annoying that the ANALYZE
	 * API requires us to return that now, because it forces some duplication
	 * of effort between this routine and maintableAcquireSampleRowsFunc.  But
...
/*
 * maintableGetAnalyzeInfoForForeignTable
 ...
 */
static double
maintableGetAnalyzeInfoForForeignTable(Relation relation, bool *can_tablesample)
{
	ForeignTable *table;
	UserMapping *user;
	PGconn	   *conn;
...
/*
 * Acquire a random sample of rows from foreign table managed by maintable_fdw.
 *
 ...
 */
static int
maintableAcquireSampleRowsFunc(Relation relation, int elevel,
							  HeapTuple *rows, int targrows,
							  double *totalrows,
							  double *totaldeadrows)
{
	...
	if (method != ANALYZE_SAMPLE_OFF)
	{
		...
		reltuples = maintableGetAnalyzeInfoForForeignTable(relation,
														  &can_tablesample);
 ...
 */
static List *
maintableImportForeignSchema(ImportForeignSchemaStmt *stmt, Oid serverOid)
{
	List	   *commands = NIL;
	...
	/* Generated columns are supported since Maintable 12 */
	...
	/* Partitions are supported since Maintable 10 */
								...
								Path *epq_path, List *restrictlist)
{
	...
	if (epq_path != NULL && useful_pathkeys_list != NIL)
	{
		 ...
		 * for the new pathtarget here.  See also maintableGetForeignPlan().
		 */
...
/*
 * maintableGetForeignJoinPaths
 *		Add possible ForeignPath to joinrel, if join is safe to push down.
 */
static void
maintableGetForeignJoinPaths(PlannerInfo *root,
							RelOptInfo *joinrel,
							RelOptInfo *outerrel,
							RelOptInfo *innerrel,
							JoinType jointype,
...
/*
 * maintableGetForeignUpperPaths
 *		Add paths for post-join operations like aggregation, grouping etc. if
 *		corresponding operations are safe to push down.
 */
static void
maintableGetForeignUpperPaths(PlannerInfo *root, UpperRelationKind stage,
						   ...
						   GroupPathExtraData *extra)
{
	 ...
	 * such as pre-sorted paths as in maintableGetForeignPaths and
	 * maintableGetForeignJoinPaths.)  The best we can do for these conditions
						  ...
						  RelOptInfo *ordered_rel)
{
	...
	/*
	 * Build the fdw_private list that will be used by maintableGetForeignPlan.
...
/*
 * maintableIsForeignPathAsyncCapable
 *		Check whether a given ForeignPath node is async-capable.
 */
static bool
maintableIsForeignPathAsyncCapable(ForeignPath *path)
{
...
/*
 * maintableForeignAsyncRequest
 *		Asynchronously request next tuple from a foreign maintableQL table.
 */
static void
maintableForeignAsyncRequest(AsyncRequest *areq)
{
	produce_tuple_asynchronously(areq, true);
}

/*
 * maintableForeignAsyncConfigureWait
 *		Configure a file descriptor event for which we wish to wait.
 */
static void
maintableForeignAsyncConfigureWait(AsyncRequest *areq)
{
...
/*
 * Asynchronously produce next tuple from a foreign maintableQL table.
 */
static void
...
process_pending_request(AsyncRequest *areq)
{
	 ...
	 * from maintableForeignAsyncConfigureWait()/maintableForeignAsyncNotify().
	 */
	if (fsstate->next_tuple >= fsstate->num_tuples)
	{
