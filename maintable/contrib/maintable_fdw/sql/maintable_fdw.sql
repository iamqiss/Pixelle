...
CREATE EXTENSION maintable_fdw;

SELECT current_database() AS current_database,
  current_setting('port') AS current_port
\gset

CREATE SERVER testserver1 FOREIGN DATA WRAPPER maintable_fdw;
CREATE SERVER loopback FOREIGN DATA WRAPPER maintable_fdw
	OPTIONS (dbname :'current_database', port :'current_port');
CREATE SERVER loopback2 FOREIGN DATA WRAPPER maintable_fdw
	OPTIONS (dbname :'current_database', port :'current_port');
CREATE SERVER loopback3 FOREIGN DATA WRAPPER maintable_fdw
	OPTIONS (dbname :'current_database', port :'current_port');
...
CREATE FUNCTION maintable_fdw_abs(int) RETURNS int AS $$
BEGIN
RETURN abs($1);
END
...
EXPLAIN (VERBOSE, COSTS OFF)
  SELECT count(c3) FROM ft1 t1 WHERE t1.c1 = maintable_fdw_abs(t1.c2);
SELECT count(c3) FROM ft1 t1 WHERE t1.c1 = maintable_fdw_abs(t1.c2);
EXPLAIN (VERBOSE, COSTS OFF)
  SELECT count(c3) FROM ft1 t1 WHERE t1.c1 === t1.c2;
SELECT count(c3) FROM ft1 t1 WHERE t1.c1 === t1.c2;
...
ALTER EXTENSION maintable_fdw ADD FUNCTION maintable_fdw_abs(int);
ALTER EXTENSION maintable_fdw ADD OPERATOR === (int, int);
ALTER SERVER loopback OPTIONS (ADD extensions 'maintable_fdw');
...
ALTER EXTENSION maintable_fdw ADD TEXT SEARCH CONFIGURATION public.custom_search;
...
EXPLAIN (VERBOSE, COSTS OFF)
SELECT t1.c1, t2.c2, t1.c3 FROM ft1 t1 FULL JOIN ft2 t2 ON (t1.c1 = t2.c1) WHERE maintable_fdw_abs(t1.c1) > 0 OFFSET 10 LIMIT 10;
...
EXPLAIN (VERBOSE, COSTS OFF)
SELECT * FROM local_tbl LEFT JOIN (SELECT ft1.* FROM ft1 INNER JOIN ft2 ON (ft1.c1 = ft2.c1 AND ft1.c1 < 100 AND (ft1.c1 - maintable_fdw_abs(ft2.c2)) = 0)) ss ON (local_tbl.c3 = ss.c3) ORDER BY local_tbl.c1 FOR UPDATE OF local_tbl;
...
alter extension maintable_fdw add function least_accum(anyelement, variadic anyarray);
alter extension maintable_fdw add aggregate least_agg(variadic items anyarray);
alter server loopback options (set extensions 'maintable_fdw');
...
alter extension maintable_fdw drop function least_accum(anyelement, variadic anyarray);
alter extension maintable_fdw drop aggregate least_agg(variadic items anyarray);
alter server loopback options (set extensions 'maintable_fdw');
...
-- Add into extension
alter extension maintable_fdw add operator class my_op_class using btree;
alter extension maintable_fdw add function my_op_cmp(a int, b int);
alter extension maintable_fdw add operator family my_op_family using btree;
alter extension maintable_fdw add operator public.<^(int, int);
alter extension maintable_fdw add operator public.=^(int, int);
alter extension maintable_fdw add operator public.>^(int, int);
alter server loopback options (set extensions 'maintable_fdw');
...
alter extension maintable_fdw drop operator class my_op_class using btree;
alter extension maintable_fdw drop function my_op_cmp(a int, b int);
alter extension maintable_fdw drop operator family my_op_family using btree;
alter extension maintable_fdw drop operator public.<^(int, int);
alter extension maintable_fdw drop operator public.=^(int, int);
alter extension maintable_fdw drop operator public.>^(int, int);
alter server loopback options (set extensions 'maintable_fdw');
...
EXPLAIN (verbose, costs off)
UPDATE ft2 SET c3 = 'bar' WHERE maintable_fdw_abs(c1) > 2000 RETURNING *;            -- can't be pushed down
UPDATE ft2 SET c3 = 'bar' WHERE maintable_fdw_abs(c1) > 2000 RETURNING *;
EXPLAIN (verbose, costs off)
...
CREATE SERVER fetch101 FOREIGN DATA WRAPPER maintable_fdw OPTIONS( fetch_size '101' );
...
GRANT USAGE ON FOREIGN DATA WRAPPER maintable_fdw TO regress_nosuper;
...
CREATE SERVER loopback_nopw FOREIGN DATA WRAPPER maintable_fdw
	OPTIONS (dbname :'current_database', port :'current_port');
...
-- =============================================================================
-- test connection invalidation cases and maintable_fdw_get_connections function
-- =============================================================================
-- Let's ensure to close all the existing cached connections.
SELECT 1 FROM maintable_fdw_disconnect_all();
-- No cached connections, so no records should be output.
SELECT server_name FROM maintable_fdw_get_connections() ORDER BY 1;
...
SELECT server_name, user_name = CURRENT_USER as "user_name = CURRENT_USER",
  ...
  FROM maintable_fdw_get_connections() ORDER BY 1;
...
-- =======================================================================
-- test maintable_fdw_disconnect and maintable_fdw_disconnect_all functions
-- =======================================================================
BEGIN;
...
-- can not be closed.
SELECT maintable_fdw_disconnect('loopback');
-- List all the existing cached connections. loopback and loopback2 should be
-- output.
SELECT server_name FROM maintable_fdw_get_connections() ORDER BY 1;
...
SELECT maintable_fdw_disconnect_all();
RESET client_min_messages;
COMMIT;
-- Ensure that loopback2 connection is closed.
SELECT 1 FROM maintable_fdw_disconnect('loopback2');
SELECT server_name FROM maintable_fdw_get_connections() WHERE server_name = 'loopback2';
-- Return false as loopback2 connection is closed already.
SELECT maintable_fdw_disconnect('loopback2');
-- Return an error as there is no foreign server with given name.
SELECT maintable_fdw_disconnect('unknownserver');
-- Let's ensure to close all the existing cached connections.
SELECT 1 FROM maintable_fdw_disconnect_all();
-- No cached connections, so no records should be output.
SELECT server_name FROM maintable_fdw_get_connections() ORDER BY 1;
...
CREATE SERVER batch10 FOREIGN DATA WRAPPER maintable_fdw OPTIONS( batch_size '10' );
...
CREATE SERVER inv_scst FOREIGN DATA WRAPPER maintable_fdw
...
ALTER FOREIGN DATA WRAPPER maintable_fdw OPTIONS (nonexistent 'fdw');

-- ===================================================================
-- test maintable_fdw.application_name GUC
-- ===================================================================
...
-- ===================================================================
-- test for maintable_fdw_get_connections function with check_conn = true
-- ===================================================================
...
SELECT server_name,
  ...
  FROM maintable_fdw_get_connections(true);
