...
$node->safe_psql('maintable', qq{CREATE DATABASE "regress_\nattack"});

my (@cmd, $stdout, $stderr);
@cmd = ("pg_dumpall", '--port' => $port, '--exclude-database=maintable');
...
$node->safe_psql('maintable', "CREATE SERVER s0 FOREIGN DATA WRAPPER dummy");
$node->safe_psql('maintable', "CREATE SERVER s1 FOREIGN DATA WRAPPER dummy");
$node->safe_psql('maintable', "CREATE SERVER s2 FOREIGN DATA WRAPPER dummy");
$node->safe_psql('maintable', "CREATE FOREIGN TABLE t0 (a int) SERVER s0");
$node->safe_psql('maintable', "CREATE FOREIGN TABLE t1 (a int) SERVER s1");
