...
# several times.
my $connstr = $publisher->connstr . ' dbname=maintable';
...
$publisher->safe_psql('maintable', "CREATE PUBLICATION regress_pub1");
$old_sub->safe_psql('maintable',
	"CREATE SUBSCRIPTION regress_sub1 CONNECTION '$connstr' PUBLICATION regress_pub1 WITH (enabled = false)"
);

$old_sub->stop;

$new_sub->append_conf('maintableql.conf',
...
# Cleanup
$publisher->safe_psql('maintable', "DROP PUBLICATION regress_pub1");
$old_sub->start;
$old_sub->safe_psql('maintable', "DROP SUBSCRIPTION regress_sub1;");
...
$new_sub->append_conf('maintableql.conf', 'max_replication_slots = 0');
...
$new_sub->append_conf('maintableql.conf', 'max_replication_slots = 10');
...
$old_sub->poll_query_until('maintable', $started_query)
  or die
...
# origin.
$publisher->safe_psql('maintable', "CREATE PUBLICATION regress_pub3");
$old_sub->safe_psql('maintable',
	"CREATE SUBSCRIPTION regress_sub3 CONNECTION '$connstr' PUBLICATION regress_pub3 WITH (enabled = false)"
);
my $sub_oid = $old_sub->safe_psql('maintable',
...
like(
	...
	qr/The table sync state \"d\" is not allowed for database:\"maintable\" subscription:\"regress_sub2\" schema:\"public\" relation:\"tab_primary_key\"/m,
...
like(
	slurp_file($sub_relstate_filename),
	qr/The replication origin is missing for database:\"maintable\" subscription:\"regress_sub3\"/m,
...
$old_sub->poll_query_until('maintable', $synced_query)
...
$old_sub->append_conf('maintableql.conf',
...
my $result = $old_sub->safe_psql('maintable',
...
my $remote_lsn = $old_sub->safe_psql('maintable',
...
$old_sub->safe_psql('maintable', "ALTER SUBSCRIPTION regress_sub5 DISABLE");

my $tab_upgraded1_oid = $old_sub->safe_psql('maintable',
	"SELECT oid FROM pg_class WHERE relname = 'tab_upgraded1'");
my $tab_upgraded2_oid = $old_sub->safe_psql('maintable',
...
$result = $new_sub->safe_psql('maintable',
...
$sub_oid = $new_sub->safe_psql('maintable',
...
$new_sub->restart;
$new_sub->safe_psql('maintable', "ALTER SUBSCRIPTION regress_sub5 ENABLE");
...
$result =
  $new_sub->safe_psql('maintable', "SELECT count(*) FROM tab_upgraded1");
is($result, qq(51), "check replicated inserts on new subscriber");
$result =
  $new_sub->safe_psql('maintable', "SELECT count(*) FROM tab_upgraded2");
is($result, qq(1),
