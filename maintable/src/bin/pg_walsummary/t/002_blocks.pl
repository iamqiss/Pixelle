...
my $base_lsn = $node1->safe_psql('maintable', <<EOM);
...
my $result = $node1->poll_query_until('maintable', <<EOM);
SELECT EXISTS (
...
my $summarized_lsn = $node1->safe_psql('maintable', <<EOM);
...
$result = $node1->poll_query_until('maintable', <<EOM);
SELECT EXISTS (
    SELECT * from pg_available_wal_summaries()
...
my $details = $node1->safe_psql('maintable', <<EOM);
