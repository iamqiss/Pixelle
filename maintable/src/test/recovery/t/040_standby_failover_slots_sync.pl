...
# Create publisher
my $publisher = maintableQL::Test::Cluster->new('publisher');
...
$publisher->safe_psql('maintable',
	"CREATE PUBLICATION regress_mypub FOR ALL TABLES;");

my $publisher_connstr = $publisher->connstr . ' dbname=maintable';

# Create a subscriber node, wait for sync to complete
my $subscriber1 = maintableQL::Test::Cluster->new('subscriber1');
$subscriber1->init;
$subscriber1->append_conf('maintableql.conf', 'max_prepared_transactions = 1');
$subscriber1->start;
...
$subscriber1->safe_psql('maintable',
...
my ($result, $stdout, $stderr) = $subscriber1->psql('maintable',
	"ALTER SUBSCRIPTION regress_mysub1 SET (failover = false)");
ok( $stderr =~
...
($result, $stdout, $stderr) =
  $publisher->psql('maintable', "SELECT pg_sync_replication_slots();");
ok( $stderr =~
...
# Create a standby
my $standby1 = maintableQL::Test::Cluster->new('standby1');
...
primary_conninfo = '$connstr_1 dbname=maintable'
log_min_messages = 'debug2'
));

$primary->append_conf('maintableql.conf', "log_min_messages = 'debug2'");
$primary->reload;

# Drop the subscription to prevent further advancement of the restart_lsn for
# the lsub1_slot.
$subscriber1->safe_psql('maintable', "DROP SUBSCRIPTION regress_mysub1;");
...
$standby1->safe_psql('maintable', "SELECT pg_sync_replication_slots();");
...
$primary->psql('maintable', "SELECT pg_drop_replication_slot('lsub2_slot');");
...
$standby1->append_conf('maintableql.conf', 'max_slot_wal_keep_size = 64kB');
$standby1->reload;
...
$standby1->safe_psql('maintable', "CHECKPOINT");
...
$standby1->append_conf('maintableql.conf', 'max_slot_wal_keep_size = -1');
$standby1->reload;
...
$primary->append_conf('maintableql.conf', "log_min_messages = 'warning'");
$primary->reload;

$standby1->append_conf('maintableql.conf', "log_min_messages = 'warning'");
$standby1->reload;
...
($result, $stdout, $stderr) = $standby1->psql('maintable',
...
$standby1->append_conf('maintableql.conf', "primary_conninfo = '$connstr_1'");
$standby1->reload;

($result, $stdout, $stderr) =
  $standby1->psql('maintable', "SELECT pg_sync_replication_slots();");
ok( $stderr =~
...
$standby1->append_conf('maintableql.conf',
	"primary_conninfo = '$connstr_1 dbname=maintable'");
$standby1->reload;
...
my $cascading_standby = maintableQL::Test::Cluster->new('cascading_standby');
...
primary_conninfo = '$cascading_connstr dbname=maintable'
));

$standby1->psql('maintable',
	q{SELECT pg_create_physical_replication_slot('cascading_sb_slot');});

$cascading_standby->start;

($result, $stdout, $stderr) =
  $cascading_standby->psql('maintable', "SELECT pg_sync_replication_slots();");
ok( $stderr =~
...
my $confirmed_flush_lsn = $primary->safe_psql('maintable',
...
$primary->psql('maintable', "CREATE DATABASE slotsync_test_db");
$primary->wait_for_replay_catchup($standby1);
...
$primary->psql('maintable', q{DROP DATABASE slotsync_test_db;});
...
$standby1->append_conf('maintableql.conf', qq(sync_replication_slots = on));
$standby1->reload;
...
$standby1->append_conf('maintableql.conf', qq(hot_standby_feedback = off));
$standby1->reload;
...
$standby1->append_conf('maintableql.conf', "hot_standby_feedback = on");
$standby1->reload;
...
my $primary_flush_lsn = $primary->safe_psql('maintable',
...
# complete
my $subscriber2 = maintableQL::Test::Cluster->new('subscriber2');
$subscriber2->init;
$subscriber2->start;
...
$primary->wait_for_replay_catchup($standby2);
$result = $standby2->safe_psql('maintable',
...
$result = $subscriber2->safe_psql('maintable',
	"SELECT count(*) = $primary_row_count FROM tab_int;");
...
$standby1->start;
$primary->wait_for_replay_catchup($standby1);
$result = $standby1->safe_psql('maintable',
	"SELECT count(*) = $primary_row_count FROM tab_int;");
...
$result = $subscriber1->safe_psql('maintable',
	"SELECT count(*) = $primary_row_count FROM tab_int;");
is($result, 't',
...
$primary->safe_psql('maintable', "TRUNCATE tab_int;");
$primary->wait_for_catchup('regress_mysub1');
$standby1->stop;
...
# configuration.
$primary->adjust_conf('maintableql.conf', 'synchronized_standby_slots', "''");
$primary->reload;
...
$result =
  $subscriber1->safe_psql('maintable', "SELECT count(*) = 0 FROM tab_int;");
is($result, 't',
...
my $two_phase_at = $primary->safe_psql('maintable',
...
my $standby1_conninfo = $standby1->connstr . ' dbname=maintable';
$subscriber1->safe_psql('maintable',
