...
my $node = maintableQL::Test::Cluster->new('n1');
...
is($node->safe_psql('maintable', 'select count(*), sum(i) FROM truncme'),
	'10|55', 'table contents as expected after recovery');
$node->safe_psql('maintable', 'DROP TABLE truncme');
...
is( $node->safe_psql(
		'maintable', 'select count(*), sum(i), min(i), max(i) FROM truncme'),
	'510|136304|2|1010',
