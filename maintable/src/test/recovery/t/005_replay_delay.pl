...
$node_standby->poll_query_until('maintable',
...
my $receive_lsn =
  $node_standby2->safe_psql('maintable', "SELECT pg_last_wal_receive_lsn()");
my $replay_lsn =
  $node_standby2->safe_psql('maintable', "SELECT pg_last_wal_replay_lsn()");
$node_primary->safe_psql('maintable',
...
is( $node_standby2->safe_psql('maintable', "SELECT pg_last_wal_replay_lsn()"),
	qq($replay_lsn),
	'no WAL is replayed in the paused state');

# Request to resume recovery and wait until it's actually resumed.
$node_standby2->safe_psql('maintable', "SELECT pg_wal_replay_resume()");
$node_standby2->poll_query_until('maintable',
...
$node_standby2->poll_query_until('maintable', "SELECT NOT pg_is_in_recovery()")
