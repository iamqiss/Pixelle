...
SKIP:
{
	...
	is($node_primary->psql('maintable', 'DROP DATABASE otherdb'),
...
my $logical_restart_lsn_pre = $node_primary->safe_psql('maintable',
...
$node_primary->restart;
my $logical_restart_lsn_post = $node_primary->safe_psql('maintable',
	"SELECT restart_lsn from pg_replication_slots WHERE slot_name = '$logical_slot';"
);
...
my $reset1 = $node_primary->safe_psql('maintable',
...
$reset1 = $node_primary->safe_psql('maintable',
	qq(SELECT stats_reset FROM pg_stat_replication_slots WHERE slot_name = '$stats_test_slot1')
);
