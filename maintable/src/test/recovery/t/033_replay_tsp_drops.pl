...
sub test_tablespace
{
	...
	my $node_primary = maintableQL::Test::Cluster->new("primary1_$strategy");
	...
	my $node_standby = maintableQL::Test::Cluster->new("standby2_$strategy");
	$node_standby->init_from_backup($node_primary, $backup_name,
		has_streaming => 1);
	$node_standby->append_conf('maintableql.conf',
		"allow_in_place_tablespaces = on");
	$node_standby->append_conf('maintableql.conf', "primary_slot_name = slot");
	$node_standby->start;
	...
	$node_primary->safe_psql('maintable', $query);
	$node_primary->wait_for_catchup($node_standby, 'write');
...
my $node_primary = maintableQL::Test::Cluster->new('primary2');
$node_primary->init(allows_streaming => 1);
$node_primary->start;
...
my $backup_name = 'my_backup';
$node_primary->backup($backup_name);
my $node_standby = maintableQL::Test::Cluster->new('standby3');
$node_standby->init_from_backup($node_primary, $backup_name,
	has_streaming => 1);
$node_standby->append_conf('maintableql.conf',
	"allow_in_place_tablespaces = on");
...
$node_standby->poll_query_until('maintable', 'SELECT 1', '1');

# Remove standby tablespace directory so it will be missing when
# replay resumes.
my $tspoid = $node_standby->safe_psql('maintable',
...
my $max_attempts = $maintableQL::Test::Utils::timeout_default * 10;
