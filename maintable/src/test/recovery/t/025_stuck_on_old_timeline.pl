...
$archivedir_primary =~ s!\\!/!g if $maintableQL::Test::Utils::windows_os;
...
my $node_cascade = maintableQL::Test::Cluster->new('cascade');
...
$node_standby->poll_query_until('maintable', "SELECT NOT pg_is_in_recovery();")
...
$node_standby->poll_query_until('maintable', $archive_wait_query)
...
# cascading standby.
$node_standby->safe_psql('maintable', "CREATE TABLE tab_int AS SELECT 1 AS a");
...
my $result =
  $node_cascade->safe_psql('maintable', "SELECT count(*) FROM tab_int");
