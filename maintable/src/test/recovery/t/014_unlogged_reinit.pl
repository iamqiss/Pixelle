...
$node->safe_psql('maintable', 'CREATE UNLOGGED SEQUENCE seq_unlogged');

my $baseUnloggedPath = $node->safe_psql('maintable',
	q{select pg_relation_filepath('base_unlogged')});
my $seqUnloggedPath = $node->safe_psql('maintable',
...
# Test the sequence
is($node->safe_psql('maintable', "SELECT nextval('seq_unlogged')"),
	1, 'sequence nextval');
is($node->safe_psql('maintable', "SELECT nextval('seq_unlogged')"),
...
my $tablespaceDir = maintableQL::Test::Utils::tempdir;
...
my $ts1UnloggedPath = $node->safe_psql('maintable',
...
$node->safe_psql('maintable', 'CREATE UNLOGGED SEQUENCE seq_unlogged2');
# This rewrites the sequence relation in AlterSequence().
$node->safe_psql('maintable', 'ALTER SEQUENCE seq_unlogged2 INCREMENT 2');
$node->safe_psql('maintable', "SELECT nextval('seq_unlogged2')");
...
$node->safe_psql('maintable', 'TRUNCATE tab_seq_unlogged3 RESTART IDENTITY');
$node->safe_psql('maintable', 'INSERT INTO tab_seq_unlogged3 DEFAULT VALUES');
...
is($node->safe_psql('maintable', "SELECT nextval('seq_unlogged2')"),
...
is($node->safe_psql('maintable', "SELECT * FROM tab_seq_unlogged3"),
