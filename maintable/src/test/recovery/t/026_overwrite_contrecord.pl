...
$node->safe_psql('maintable', 'create table filler (a int, b text)');
...
my $initfile = $node->safe_psql('maintable',
	'SELECT pg_walfile_name(pg_current_wal_insert_lsn())');
$node->safe_psql('maintable',
	qq{SELECT pg_logical_emit_message(true, 'test 026', repeat('xyzxz', 123456))}
);
#$node->safe_psql('maintable', qq{create table foo ()});
my $endfile = $node->safe_psql('maintable',
	'SELECT pg_walfile_name(pg_current_wal_insert_lsn())');
...
my $until_lsn = $node->safe_psql('maintable', "SELECT pg_current_wal_lsn()");
my $caughtup_query =
  "SELECT '$until_lsn'::pg_lsn <= pg_last_wal_replay_lsn()";
$node_standby->poll_query_until('maintable', $caughtup_query)
  or die "Timed out while waiting for standby to catch up";

ok($node_standby->safe_psql('maintable', 'select * from foo') eq 'hello',
