...
# enabled.
my $node = maintableQL::Test::Cluster->new('orig');
...
$node->append_conf('maintableql.conf', $replica_config);
$node->start;
...
my $walfile_to_be_archived = $node->safe_psql('maintable',
...
$node->poll_query_until('maintable', $archive_wait_query)
...
sub test_recovery_wal_level_minimal
{
	...
	my $recovery_node = maintableQL::Test::Cluster->new($node_name);
