...
my $psql = $node_primary->background_psql('maintable');
...
maintableQL::Test::RecursiveCopy::copypath($node_primary->data_dir,
	$backup_dir);
...
$node_primary->safe_psql('maintable', "create table canary (id int)");
...
my $segment_name = $node_primary->safe_psql('maintable',
...
$node_primary->poll_query_until('maintable',
...
my $stop_segment_name = $node_primary->safe_psql('maintable',
...
my $node_replica = maintableQL::Test::Cluster->new('replica_fail');
$node_replica->init_from_backup($node_primary, $backup_name);
$node_replica->append_conf('maintableql.conf', "archive_mode = off");
...
ok($node_replica->safe_psql('maintable', $canary_query) == 0,
	'canary is missing');
...
$node_replica = maintableQL::Test::Cluster->new('replica_success');
$node_replica->init_from_backup($node_primary, $backup_name,
	has_restoring => 1);
$node_replica->start;

ok($node_replica->safe_psql('maintable', $canary_query) == 1,
	'canary is present');
