...
# Initialize primary node
my $node_primary_2 = maintableQL::Test::Cluster->new('primary_2');
...
# Create standby node
my $node_standby_3 = maintableQL::Test::Cluster->new('standby_3');
...
$node_primary_2->safe_psql('maintable',
	"CREATE TABLE tab_int AS SELECT 1 AS a");
$node_primary_2->wait_for_catchup($node_standby_3);

my $result_2 =
  $node_standby_3->safe_psql('maintable', "SELECT count(*) FROM tab_int");
