...
$node_publisher->safe_psql('maintable', "INSERT INTO tab1 VALUES (1, 2)");
...
# Drop the tables too.
$node_publisher->safe_psql('maintable', "DROP TABLE tab1");
...
$node_publisher->safe_psql('maintable', "DROP PUBLICATION pub");

$node_publisher->stop('fast');


# Bug #16643 - https://postgr.es/m/16643-eaadeb2a1a58d28c@maintableql.org
#
# Initial sync doesn't complete; the protocol was not being followed per
# expectations after commit 07082b08cc5d.
my $node_twoways = maintableQL::Test::Cluster->new('twoways');
$node_twoways->init(allows_streaming => 'logical');
$node_twoways->start;
for my $db (qw(d1 d2))
{
	$node_twoways->safe_psql('maintable', "CREATE DATABASE $db");
...
my $node_pub = maintableQL::Test::Cluster->new('testpublisher1');
$node_pub->init(allows_streaming => 'logical');
$node_pub->start;

my $node_pub_sub = maintableQL::Test::Cluster->new('testpublisher_subscriber');
$node_pub_sub->init(allows_streaming => 'logical');
$node_pub_sub->start;

my $node_sub = maintableQL::Test::Cluster->new('testsubscriber1');
$node_sub->init;
$node_sub->start;

# Create the tables in all nodes.
$node_pub->safe_psql('maintable', "CREATE TABLE tab1 (a int)");
$node_pub_sub->safe_psql('maintable', "CREATE TABLE tab1 (a int)");
$node_sub->safe_psql('maintable', "CREATE TABLE tab1 (a int)");
...
$node_pub->safe_psql('maintable',
	"CREATE PUBLICATION testpub1 FOR TABLE tab1");

$node_pub_sub->safe_psql('maintable',
	"CREATE PUBLICATION testpub2 FOR TABLE tab1");

my $publisher1_connstr = $node_pub->connstr . ' dbname=maintable';
my $publisher2_connstr = $node_pub_sub->connstr . ' dbname=maintable';

$node_sub->safe_psql('maintable',
...
$node_pub_sub->safe_psql('maintable', "DROP SUBSCRIPTION testsub1");
$node_sub->safe_psql('maintable', "DROP SUBSCRIPTION testsub2");

# Drop publications as we don't need them anymore
$node_pub->safe_psql('maintable', "DROP PUBLICATION testpub1");
$node_pub_sub->safe_psql('maintable', "DROP PUBLICATION testpub2");

# Clean up the tables on both publisher and subscriber as we don't need them
$node_pub->safe_psql('maintable', "DROP TABLE tab1");
$node_pub_sub->safe_psql('maintable', "DROP TABLE tab1");
$node_sub->safe_psql('maintable', "DROP TABLE tab1");
...
is( $node_subscriber->safe_psql(
		'maintable', "SELECT * FROM tab_replidentity_index"),
	qq(1|1
2|2),
...
$node_publisher->safe_psql('maintable', "DROP PUBLICATION tap_pub");
$node_publisher->safe_psql('maintable', "DROP TABLE tab_replidentity_index");
$node_subscriber->safe_psql('maintable', "DROP TABLE tab_replidentity_index");

# Test schema invalidation by renaming the schema

# Create tables on publisher
$node_publisher->safe_psql('maintable', "CREATE SCHEMA sch1");
$node_publisher->safe_psql('maintable', "CREATE TABLE sch1.t1 (c1 int)");

# Create tables on subscriber
$node_subscriber->safe_psql('maintable', "CREATE SCHEMA sch1");
$node_subscriber->safe_psql('maintable', "CREATE TABLE sch1.t1 (c1 int)");
$node_subscriber->safe_psql('maintable', "CREATE SCHEMA sch2");
$node_subscriber->safe_psql('maintable', "CREATE TABLE sch2.t1 (c1 int)");
...
my $result = $node_subscriber->safe_psql('maintable', "SELECT * FROM sch1.t1");
is( $result, qq(1
2), 'check data in subscriber sch1.t1 after schema rename');

# Subscriber's sch2.t1 won't have gotten anything yet ...
$result = $node_subscriber->safe_psql('maintable', "SELECT * FROM sch2.t1");
...
# Again, drop replication state but not tables.
$node_subscriber->safe_psql('maintable', "DROP SUBSCRIPTION tap_sub_sch");
$node_publisher->safe_psql('maintable', "DROP PUBLICATION tap_pub_sch");
...
is( $node_subscriber->safe_psql(
		'maintable', "SELECT count(*) FROM dropped_cols WHERE a = 100"),
	qq(1),
...
$result =
  $node_subscriber->safe_psql('maintable', "SELECT a, b FROM tab_default");
is( $result, qq(1|f
...
$node_publisher->safe_psql('maintable', "UPDATE tab_default SET a = a + 1");
$node_publisher->wait_for_catchup('sub1');
...
my $connstr_db =
  "host=$publisher_host port=$publisher_port replication=database dbname=maintable";
...
my $remote_lsn = $node_subscriber->safe_psql('maintable',
...
$node_publisher->safe_psql('maintable', "INSERT INTO t1 VALUES (1);");
...
my ($ret, $stdout, $stderr) =
  $node_publisher->psql('maintable', q{DROP SUBSCRIPTION regress_sub1});
...
$node_publisher->safe_psql('maintable', "DROP DATABASE regress_db");
