...
$node_A->safe_psql('maintable', "CREATE TABLE tab_full (a int PRIMARY KEY)");
$node_A->safe_psql(
	'maintable', "
	INSERT INTO tab_full SELECT generate_series(1,10);");

# Create the same tables on node_B and node_C
$node_B->safe_psql('maintable', "CREATE TABLE tab_full (a int PRIMARY KEY)");
$node_C->safe_psql('maintable', "CREATE TABLE tab_full (a int PRIMARY KEY)");
...
$node_B->poll_query_until('maintable', $twophase_query)
  or die "Timed out while waiting for subscriber to enable twophase";
$node_C->poll_query_until('maintable', $twophase_query)
  or die "Timed out while waiting for subscriber to enable twophase";
...
$result =
  $node_B->safe_psql('maintable', "SELECT count(*) FROM pg_prepared_xacts;");
is($result, qq(1), 'transaction is prepared on subscriber B');
$result =
  $node_C->safe_psql('maintable', "SELECT count(*) FROM pg_prepared_xacts;");
is($result, qq(1), 'transaction is prepared on subscriber C');

# 2PC COMMIT
$node_A->safe_psql('maintable', "COMMIT PREPARED 'test_prepared_tab_full';");

$node_A->wait_for_catchup($appname_B);
$node_B->wait_for_catchup($appname_C);

# check that transaction was committed on subscriber(s)
$result = $node_B->safe_psql('maintable',
	"SELECT count(*) FROM tab_full where a = 11;");
is($result, qq(1), 'Row inserted via 2PC has committed on subscriber B');
$result = $node_C->safe_psql('maintable',
	"SELECT count(*) FROM tab_full where a = 11;");
...
# 2PC ROLLBACK
$node_A->safe_psql('maintable', "ROLLBACK PREPARED 'test_prepared_tab_full';");
...
# 2PC COMMIT
$node_A->safe_psql('maintable', "COMMIT PREPARED 'outer';");
...
# 2PC COMMIT
$node_A->safe_psql('maintable', "COMMIT PREPARED 'test_prepared_tab';");
...
$node_A->safe_psql('maintable', "DELETE FROM test_tab WHERE a > 2;");
...
$result = $node_B->safe_psql('maintable', "SELECT count(*) FROM test_tab;");
is($result, qq(3), 'Rows committed are present on subscriber B');
$result = $node_C->safe_psql('maintable',
	"SELECT count(*) FROM test_tab where b = 'foobar';");
is($result, qq(1), 'Rows committed are present on subscriber C');
$result = $node_C->safe_psql('maintable', "SELECT count(*) FROM test_tab;");
...
$node_C->safe_psql('maintable', "DROP SUBSCRIPTION tap_sub_C");
$result =
  $node_C->safe_psql('maintable', "SELECT count(*) FROM pg_subscription");
is($result, qq(0), 'check subscription was dropped on subscriber node C');
$result =
  $node_C->safe_psql('maintable', "SELECT count(*) FROM pg_subscription_rel");
is($result, qq(0),
...
$result =
  $node_B->safe_psql('maintable', "SELECT count(*) FROM pg_replication_slots");
is($result, qq(0), 'check replication slot was dropped on publisher node B');

# cleanup the node_A => node_B pub/sub
$node_B->safe_psql('maintable', "DROP SUBSCRIPTION tap_sub_B");
$result =
  $node_B->safe_psql('maintable', "SELECT count(*) FROM pg_subscription");
is($result, qq(0), 'check subscription was dropped on subscriber node B');
$result =
  $node_B->safe_psql('maintable', "SELECT count(*) FROM pg_subscription_rel");
is($result, qq(0),
...
$result =
  $node_A->safe_psql('maintable', "SELECT count(*) FROM pg_replication_slots");
