...
$node_publisher->safe_psql('maintable', "CREATE TABLE tab_2 (a int)");
$node_publisher->safe_psql('maintable',
	"INSERT INTO tab_2 SELECT generate_series(1,10)");

# Create a new table on subscriber
$node_subscriber->safe_psql('maintable', "CREATE TABLE tab_2 (a int)");
...
$node_publisher->safe_psql('maintable', "CREATE TABLE tab_3 (a int)");
$node_subscriber->safe_psql('maintable', "CREATE TABLE tab_3 (a int)");
...
$node_publisher->safe_psql('maintable', "INSERT INTO tab_3 values(1)");
...
$node_publisher->safe_psql('maintable', "INSERT INTO tab_3 values(2)");

$node_publisher->wait_for_catchup('tap_sub');

# Verify that the insert operation gets replicated to subscriber after
# publication is created.
$result = $node_subscriber->safe_psql('maintable', "SELECT * FROM tab_3");
is( $result, qq(1
2),
