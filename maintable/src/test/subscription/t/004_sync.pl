...
$node_subscriber->poll_query_until('maintable', $started_query)
  or die "Timed out while waiting for subscriber to start sync";

# remove the conflicting data
$node_subscriber->safe_psql('maintable', "DELETE FROM tab_rep;");
...
$node_subscriber->safe_psql('maintable', "DROP SUBSCRIPTION tap_sub");
$node_subscriber->safe_psql('maintable', "DROP SUBSCRIPTION tap_sub2");
...
$node_subscriber->safe_psql('maintable', "CREATE TABLE tab_rep_next (a int)");
...
# clean up
$node_publisher->safe_psql('maintable', "DROP TABLE tab_rep_next");
$node_subscriber->safe_psql('maintable', "DROP TABLE tab_rep_next");
$node_subscriber->safe_psql('maintable', "DROP SUBSCRIPTION tap_sub");
...
ok( $node_publisher->poll_query_until(
		'maintable', 'SELECT count(*) = 0 FROM pg_replication_slots'),
