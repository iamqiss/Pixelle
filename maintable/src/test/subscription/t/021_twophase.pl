...
$node_subscriber->poll_query_until('maintable', $twophase_query)
...
# COMMIT
$node_publisher->safe_psql('maintable', "COMMIT PREPARED 'outer';");
...
# ROLLBACK
$node_publisher->safe_psql('maintable', "ROLLBACK PREPARED '';");
...
$node_subscriber->safe_psql('maintable',
	"CREATE TABLE tab_copy (a int PRIMARY KEY)");
$node_subscriber->safe_psql('maintable', "INSERT INTO tab_copy VALUES (88);");
$result =
  $node_subscriber->safe_psql('maintable', "SELECT count(*) FROM tab_copy;");
...
$node_publisher->safe_psql('maintable', "COMMIT PREPARED 'mygid';");

$result =
  $node_publisher->safe_psql('maintable', "SELECT count(*) FROM tab_copy;");
...
# Now commit the insert and verify that it is replicated.
###############################
$node_publisher->safe_psql('maintable', "COMMIT PREPARED 'newgid';");
...
$node_subscriber->safe_psql('maintable', "DROP SUBSCRIPTION tap_sub_copy;");
$node_publisher->safe_psql('maintable', "DROP PUBLICATION tap_pub_copy;");
