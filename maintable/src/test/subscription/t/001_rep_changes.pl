...
$node_publisher->safe_psql('maintable', "CREATE TABLE tab_full2 (x text)");
$node_publisher->safe_psql('maintable',
...
$node_publisher->safe_psql('maintable', "CREATE TABLE tab_nothing (a int)");
$node_publisher->safe_psql('maintable',
...
$node_publisher->safe_psql('maintable', "CREATE TABLE tab_no_col()");
$node_publisher->safe_psql('maintable',
	"INSERT INTO tab_no_col default VALUES");

# Setup structure on subscriber
$node_subscriber->safe_psql('maintable', "CREATE TABLE tab_notrep (a int)");
$node_subscriber->safe_psql('maintable', "CREATE TABLE tab_ins (a int)");
$node_subscriber->safe_psql('maintable', "CREATE TABLE tab_full (a int)");
$node_subscriber->safe_psql('maintable', "CREATE TABLE tab_full2 (x text)");
...
$node_subscriber->safe_psql('maintable',
	"ALTER TABLE tab_full_pk REPLICA IDENTITY FULL");
$node_subscriber->safe_psql('maintable', "CREATE TABLE tab_nothing (a int)");
...
$node_subscriber->safe_psql('maintable', "CREATE TABLE tab_no_col()");

# Setup logical replication
my $publisher_connstr = $node_publisher->connstr . ' dbname=maintable';
$node_publisher->safe_psql('maintable', "CREATE PUBLICATION tap_pub");
$node_publisher->safe_psql('maintable',
...
$node_publisher->safe_psql('maintable', "SELECT pg_stat_reset_shared('io')");

my $result =
  $node_subscriber->safe_psql('maintable', "SELECT count(*) FROM tab_notrep");
is($result, qq(0), 'check non-replicated table is empty on subscriber');

$result =
  $node_subscriber->safe_psql('maintable', "SELECT count(*) FROM tab_ins");
is($result, qq(1002), 'check initial data was copied to subscriber');

$node_publisher->safe_psql('maintable',
	"INSERT INTO tab_ins SELECT generate_series(1,50)");
$node_publisher->safe_psql('maintable', "DELETE FROM tab_ins WHERE a > 20");
$node_publisher->safe_psql('maintable', "UPDATE tab_ins SET a = -a");

$node_publisher->safe_psql('maintable',
	"INSERT INTO tab_rep SELECT generate_series(1,50)");
$node_publisher->safe_psql('maintable', "DELETE FROM tab_rep WHERE a > 20");
$node_publisher->safe_psql('maintable', "UPDATE tab_rep SET a = -a");
...
$node_publisher->safe_psql('maintable', "UPDATE tab_include SET a = -a");
...
$result = $node_subscriber->safe_psql('maintable', "SELECT * FROM tab_mixed");
is( $result, qq(local|1.1|foo|1
local|2.2|bar|2), 'check replicated changes with different column order');

$result =
  $node_subscriber->safe_psql('maintable', "SELECT count(*) FROM tab_nothing");
...
is( $node_subscriber->safe_psql(
		'maintable', q(SELECT c1 FROM tab_no_replidentity_index)),
	1,
	"value replicated to subscriber without replica identity index");

$result =
  $node_subscriber->safe_psql('maintable', "SELECT count(*) FROM tab_no_col");
...
$node_publisher->safe_psql('maintable', "INSERT INTO tab_ins VALUES(8888)");
...
$node_publisher->safe_psql('maintable', "DELETE FROM tab_ins WHERE a = 8888");
...
$node_publisher->safe_psql('maintable', "CREATE TABLE temp1 (a int)");
$node_publisher->safe_psql('maintable', "CREATE TABLE temp2 (a int)");

# Create tables on subscriber
$node_subscriber->safe_psql('maintable', "CREATE TABLE temp1 (a int)");
$node_subscriber->safe_psql('maintable', "CREATE TABLE temp2 (a int)");
...
$result =
  $node_subscriber->safe_psql('maintable', "SELECT count(*) FROM temp1");
is($result, qq(0),
...
$node_publisher->safe_psql('maintable', "INSERT INTO temp1 VALUES (1)");
...
$node_subscriber->safe_psql('maintable', "DROP SUBSCRIPTION tap_sub_temp1");

# Drop publications as we don't need them anymore
$node_publisher->safe_psql('maintable', "DROP PUBLICATION tap_pub_temp1");
$node_publisher->safe_psql('maintable', "DROP PUBLICATION tap_pub_temp2");

# Clean up the tables on both publisher and subscriber as we don't need them
$node_publisher->safe_psql('maintable', "DROP TABLE temp1");
$node_publisher->safe_psql('maintable', "DROP TABLE temp2");
$node_subscriber->safe_psql('maintable', "DROP TABLE temp1");
$node_subscriber->safe_psql('maintable', "DROP TABLE temp2");
...
# and do the updates
$node_publisher->safe_psql('maintable', "UPDATE tab_full SET a = a * a");
$node_publisher->safe_psql('maintable',
...
$node_subscriber->safe_psql('maintable', "DELETE FROM tab_full_pk");
$node_subscriber->safe_psql('maintable', "DELETE FROM tab_full WHERE a = 25");
...
$node_publisher->safe_psql('maintable', "DELETE FROM tab_full_pk WHERE a = 2");
...
$node_publisher->safe_psql('maintable', "ALTER TABLE tab_mixed DROP COLUMN b");
...
$oldpid = $node_publisher->safe_psql('maintable',
	"SELECT pid FROM pg_stat_replication WHERE application_name = 'tap_sub' AND state = 'streaming';"
);
$node_subscriber->safe_psql('maintable',
...
$node_publisher->safe_psql('maintable', "DELETE FROM tab_rep");
...
$node_publisher->safe_psql('maintable', "DELETE FROM tab_ins WHERE a > 0");
$node_subscriber->safe_psql('maintable',
	"ALTER SUBSCRIPTION tap_sub REFRESH PUBLICATION WITH (copy_data = false)"
);
$node_publisher->safe_psql('maintable', "INSERT INTO tab_full VALUES(0)");
...
$node_publisher->append_conf('maintableql.conf', "log_min_messages = debug1");
...
$node_publisher->safe_psql('maintable', "INSERT INTO tab_notrep VALUES (11)");
...
$node_publisher->append_conf('maintableql.conf', "log_min_messages = warning");
$node_publisher->reload;
...
$node_subscriber->safe_psql('maintable', "DROP SUBSCRIPTION tap_sub_renamed");

$result = $node_subscriber->safe_psql('maintable',
	"SELECT count(*) FROM pg_subscription");
is($result, qq(0), 'check subscription was dropped on subscriber');

$result = $node_publisher->safe_psql('maintable',
