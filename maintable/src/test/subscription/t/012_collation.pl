...
$node_publisher->safe_psql('maintable', q{CREATE TABLE tab2 (a text, b text)});
$node_publisher->safe_psql('maintable',
...
is($node_subscriber->safe_psql('maintable', q{SELECT b FROM tab1}),
...
is($node_subscriber->safe_psql('maintable', q{SELECT b FROM tab2}),
	qq(bar),
