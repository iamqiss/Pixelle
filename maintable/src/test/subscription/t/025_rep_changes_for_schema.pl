...
$node_subscriber->safe_psql('maintable', "CREATE TABLE sch1.tab1 (a int)");
$node_subscriber->safe_psql('maintable', "CREATE TABLE sch1.tab2 (a int)");
...
$node_subscriber->safe_psql('maintable', "CREATE TABLE sch1.tab3(a int)");

$node_publisher->wait_for_catchup('tap_sub_schema');

$result =
  $node_subscriber->safe_psql('maintable', "SELECT count(*) FROM sch1.tab3");
...
$node_publisher->safe_psql('maintable', "INSERT INTO sch1.tab3 VALUES(11)");
...
$node_publisher->safe_psql('maintable', "INSERT INTO public.tab3 VALUES(12)");
...
$node_publisher->safe_psql('maintable', "DROP TABLE sch1.tab2");
$node_publisher->wait_for_catchup('tap_sub_schema');
$result = $node_subscriber->safe_psql('maintable',
	"SELECT count(*) FROM pg_subscription_rel WHERE srsubid IN (SELECT oid FROM pg_subscription WHERE subname = 'tap_sub_schema')"
);
