...
$node_subscriber->safe_psql('maintable', "ALTER SEQUENCE seq1 START 101");
...
my $result = $node_subscriber->safe_psql('maintable',
	"SELECT count(*), min(a), max(a) FROM tab1");
is($result, qq(0||), 'truncate replicated');

$result = $node_subscriber->safe_psql('maintable', "SELECT nextval('seq1')");
is($result, qq(1), 'sequence not restarted');

# truncate with restart identity

$node_publisher->safe_psql('maintable', "TRUNCATE tab1 RESTART IDENTITY");
...
$node_publisher->safe_psql('maintable', "TRUNCATE tab2");
...
$node_publisher->safe_psql('maintable', "TRUNCATE tab3, tab4");
...
$node_subscriber->safe_psql('maintable', "DROP SUBSCRIPTION sub2");
$node_publisher->safe_psql('maintable', "DROP PUBLICATION pub2");
...
$node_publisher->safe_psql('maintable', "TRUNCATE tab1, tab2");
...
$node_publisher->safe_psql('maintable', "SELECT pg_reload_conf()");
...
$node_publisher->safe_psql('maintable', "CREATE TABLE tab5 (a int)");

$node_subscriber->safe_psql('maintable', "CREATE TABLE tab5 (a int)");
...
$node_publisher->safe_psql('maintable', "TRUNCATE tab5");
...
# check deadlocks
$result = $node_subscriber->safe_psql('maintable',
	"SELECT deadlocks FROM pg_stat_database WHERE datname='maintable'");
