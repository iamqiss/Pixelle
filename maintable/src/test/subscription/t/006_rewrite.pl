...
is( $node_subscriber->safe_psql('maintable', q{SELECT a, b FROM test1}),
	qq(1|one
2|two),
...
$node_subscriber->safe_psql('maintable', $ddl2);
$node_publisher->safe_psql('maintable', $ddl2);
...
is( $node_subscriber->safe_psql('maintable', q{SELECT a, b, c FROM test1}),
	qq(1|one|0
2|two|0
3|three|33),
