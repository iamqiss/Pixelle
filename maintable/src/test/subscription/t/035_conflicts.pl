...
$node_subscriber->safe_psql('maintable', "TRUNCATE conf_tab;");
...
$node_A->safe_psql('maintable', "CREATE TABLE tab (a int PRIMARY KEY, b int)");

# Create the same table on node_B
$node_B->safe_psql('maintable', "CREATE TABLE tab (a int PRIMARY KEY, b int)");
...
my ($cmdret, $stdout, $stderr) = $node_A->psql('maintable',
...
$node_A->psql('maintable', "ALTER SUBSCRIPTION $subname_AB DISABLE;");

# Wait for the apply worker to stop
$node_A->poll_query_until('maintable',
...
($cmdret, $stdout, $stderr) = $node_A->psql('maintable',
	"ALTER SUBSCRIPTION $subname_AB SET (retain_dead_tuples = true);");
ok( $stderr =~
...
$node_A->safe_psql('maintable', "ALTER SUBSCRIPTION $subname_AB ENABLE;");
...
# Insert a record
$node_A->safe_psql('maintable', "INSERT INTO tab VALUES (1, 1), (2, 2);");
$node_A->wait_for_catchup($subname_BA);

my $result = $node_B->safe_psql('maintable', "SELECT * FROM tab;");
is($result, qq(1|1
2|2), 'check replicated insert on node B');

# Disable the logical replication from node B to node A
$node_A->safe_psql('maintable', "ALTER SUBSCRIPTION $subname_AB DISABLE");
...
$node_B->safe_psql('maintable', "UPDATE tab SET b = 3 WHERE a = 1;");
$node_A->safe_psql('maintable', "DELETE FROM tab WHERE a = 1;");

($cmdret, $stdout, $stderr) = $node_A->psql(
	'maintable', qq(VACUUM (verbose) public.tab;)
);
...
$node_A->safe_psql(
	'maintable', "ALTER SUBSCRIPTION $subname_AB ENABLE;");
...
my $next_xid = $node_A->safe_psql('maintable', "SELECT txid_current() + 1;");
...
$node_A->safe_psql('maintable', "ALTER TABLE tab REPLICA IDENTITY FULL");
$node_B->safe_psql('maintable', "ALTER TABLE tab REPLICA IDENTITY FULL");
$node_A->safe_psql('maintable', "ALTER TABLE tab DROP CONSTRAINT tab_pkey;");
...
$node_B->safe_psql('maintable', "UPDATE tab SET b = 4 WHERE a = 2;");
$node_A->safe_psql('maintable', "DELETE FROM tab WHERE a = 2;");
...
$node_B->safe_psql(
	'maintable', "DROP SUBSCRIPTION $subname_BA");
...
$node_A->safe_psql(
	'maintable', "DROP SUBSCRIPTION $subname_AB");
