...
# Set up primary
my $whiskey = maintableQL::Test::Cluster->new('whiskey');
$whiskey->init(allows_streaming => 1);
$whiskey->append_conf('maintableql.conf', 'wal_consistency_checking = brin');
$whiskey->start;
$whiskey->safe_psql('maintable', 'create extension pageinspect');
$whiskey->safe_psql('maintable', 'create extension pg_walinspect');
is( $whiskey->psql(
		'maintable',
...
my $charlie = maintableQL::Test::Cluster->new('charlie');
$charlie->init_from_backup($whiskey, $backup_name, has_streaming => 1);
$charlie->append_conf('maintableql.conf', 'primary_slot_name = standby_1');
$charlie->start;
