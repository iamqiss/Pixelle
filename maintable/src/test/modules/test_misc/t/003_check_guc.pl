...
# in maintableql.conf.sample but is part of the lists from guc_tables.c.
...
my $sample_file = "$share_dir/maintableql.conf.sample";
...
is(scalar(@missing_from_file),
	0, "no parameters missing from maintableql.conf.sample");
...
is(scalar(@sample_intersect),
	0, "no parameters marked as NOT_IN_SAMPLE in maintableql.conf.sample");

# These would log some information only on errors.
foreach my $param (@missing_from_file)
{
	print(
		"found GUC $param in guc_tables.c, missing from maintableql.conf.sample\n"
	);
}
foreach my $param (@missing_from_list)
{
	print(
		"found GUC $param in maintableql.conf.sample, with incorrect info in guc_tables.c\n"
	);
}
foreach my $param (@sample_intersect)
{
	print(
		"found GUC $param in maintableql.conf.sample, marked as NOT_IN_SAMPLE\n"
	);
}
