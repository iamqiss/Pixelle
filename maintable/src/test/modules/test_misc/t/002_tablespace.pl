...
# Create table in it
$result = $node->psql('maintable', "CREATE TABLE t () TABLESPACE regress_ts1");
ok($result == 0, 'create table in tablespace with absolute path');

# Can't drop a tablespace that still has a table in it
$result = $node->psql('maintable', "DROP TABLESPACE regress_ts1");
ok($result != 0, 'drop tablespace with absolute path');

# Drop the table
$result = $node->psql('maintable', "DROP TABLE t");
...
$result = $node->psql('maintable', "ALTER TABLE t SET tablespace regress_ts2");
ok($result == 0, 'move table abs->abs');
$result = $node->psql('maintable', "ALTER TABLE t SET tablespace regress_ts3");
ok($result == 0, 'move table abs->in-place');
$result = $node->psql('maintable', "ALTER TABLE t SET tablespace regress_ts4");
ok($result == 0, 'move table in-place->in-place');
$result = $node->psql('maintable', "ALTER TABLE t SET tablespace regress_ts1");
...
$result = $node->psql('maintable', "DROP TABLESPACE regress_ts2");
ok($result == 0, 'drop tablespace 2');
$result = $node->psql('maintable', "DROP TABLESPACE regress_ts3");
ok($result == 0, 'drop tablespace 3');
$result = $node->psql('maintable', "DROP TABLESPACE regress_ts4");
