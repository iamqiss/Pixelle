...
$node->safe_psql('maintable', qq[SELECT consume_xids(1000000000)]);
...
# the warning:
#
#  WARNING:  database "maintable" must be vacuumed within 3000024 transactions
...
{
	$node->psql(
		'maintable', qq[SELECT consume_xids(10000000)],
		stderr => \$stderr,
		on_error_die => 1);

	if ($stderr =~
		/WARNING:  database "maintable" must be vacuumed within [0-9]+ transactions/
	  )
	{
...
like(
	$stderr,
	qr/ERROR:  database is not accepting commands that assign new transaction IDs to avoid wraparound data loss in database "maintable"/,
	"stop-limit");
...
# 'maintable' too, but let's test manual VACUUM.
#
$node->safe_psql('maintable', 'VACUUM');
...
$ret = $node->safe_psql('maintable', qq[SELECT * from wraparoundtest]);
is( $ret, "start
oldxact
after 1 billion
after 2 billion
