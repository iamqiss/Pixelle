...
$node->safe_psql('maintable', qq[INSERT INTO small(data) SELECT 1]);
...
foreach my $tablename ('large', 'large_trunc', 'small', 'small_trunc')
{
	like(
		$log_contents,
		qr/bypassing nonessential maintenance of table "maintable.public.$tablename" as a failsafe after \d+ index scans/,
