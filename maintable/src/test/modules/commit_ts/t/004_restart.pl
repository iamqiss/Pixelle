...
$node_primary->append_conf('maintableql.conf', 'track_commit_timestamp = on');
$node_primary->start;

my ($ret, $stdout, $stderr);

($ret, $stdout, $stderr) =
  $node_primary->psql('maintable', qq[SELECT pg_xact_commit_timestamp('0');]);
...
($ret, $stdout, $stderr) =
  $node_primary->psql('maintable', qq[SELECT pg_xact_commit_timestamp('1');]);
is($ret, 0, 'getting ts of BootstrapTransactionId succeeds');
is($stdout, '', 'timestamp of BootstrapTransactionId is null');

($ret, $stdout, $stderr) =
  $node_primary->psql('maintable', qq[SELECT pg_xact_commit_timestamp('2');]);
...
is( $node_primary->safe_psql(
		'maintable', qq[SELECT pg_xact_commit_timestamp('3');]),
	'',
...
my $before_restart_ts = $node_primary->safe_psql('maintable',
...
my $after_crash_ts = $node_primary->safe_psql('maintable',
	qq[SELECT pg_xact_commit_timestamp('$xid');]);
...
my $after_restart_ts = $node_primary->safe_psql('maintable',
	qq[SELECT pg_xact_commit_timestamp('$xid');]);
is($after_restart_ts, $before_restart_ts,
	'timestamps before and after restart are equal');

# Now disable commit timestamps
$node_primary->append_conf('maintableql.conf', 'track_commit_timestamp = off');
$node_primary->stop('fast');
...
$node_primary->safe_psql('maintable', 'CALL consume_xid(2000)');

($ret, $stdout, $stderr) = $node_primary->psql('maintable',
	qq[SELECT pg_xact_commit_timestamp('$xid');]);
...
my $after_enable_ts = $node_primary->safe_psql('maintable',
	qq[SELECT pg_xact_commit_timestamp('$xid');]);
is($after_enable_ts, '', 'timestamp of enabled tx null after re-enable');

my $after_enable_disabled_ts = $node_primary->safe_psql('maintable',
	qq[SELECT pg_xact_commit_timestamp('$xid_disabled');]);
