...
for my $i (1 .. 10)
{
	$primary->safe_psql('maintable', "create table t$i()");
}
my $primary_ts = $primary->safe_psql('maintable',
	qq{SELECT ts.* FROM pg_class, pg_xact_commit_timestamp(xmin) AS ts WHERE relname = 't10'}
);
my $primary_lsn =
  $primary->safe_psql('maintable', 'select pg_current_wal_lsn()');
$standby->poll_query_until('maintable',
	qq{SELECT '$primary_lsn'::pg_lsn <= pg_last_wal_replay_lsn()})
  or die "standby never caught up";

my $standby_ts = $standby->safe_psql('maintable',
...
$primary->append_conf('maintableql.conf', 'track_commit_timestamp = off');
$primary->restart;
$primary->safe_psql('maintable', 'checkpoint');
$primary_lsn = $primary->safe_psql('maintable', 'select pg_current_wal_lsn()');
$standby->poll_query_until('maintable',
	qq{SELECT '$primary_lsn'::pg_lsn <= pg_last_wal_replay_lsn()})
  or die "standby never caught up";
...
my ($ret, $standby_ts_stdout, $standby_ts_stderr) = $standby->psql('maintable',
