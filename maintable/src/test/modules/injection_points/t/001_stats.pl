...
$node->safe_psql('maintable', "SELECT injection_points_run('stats-notice');");
$node->safe_psql('maintable', "SELECT injection_points_run('stats-notice');");
my $numcalls = $node->safe_psql('maintable',
	"SELECT injection_points_stats_numcalls('stats-notice');");
is($numcalls, '2', 'number of stats calls');
my $fixedstats = $node->safe_psql('maintable',
...
$numcalls = $node->safe_psql('maintable',
	"SELECT injection_points_stats_numcalls('stats-notice');");
is($numcalls, '2', 'number of stats calls');
$node->safe_psql('maintable', "SELECT injection_points_stats_drop();");
$numcalls = $node->safe_psql('maintable',
	"SELECT injection_points_stats_numcalls('stats-notice');");
...
$node->stop;
$node->adjust_conf('maintableql.conf', 'shared_preload_libraries', "''");
$node->start;
