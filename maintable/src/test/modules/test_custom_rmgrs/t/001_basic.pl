...
# setup
$node->safe_psql('maintable', 'CREATE EXTENSION test_custom_rmgrs');
...
$node->safe_psql('maintable', 'CREATE EXTENSION pg_walinspect');

# make sure checkpoints don't interfere with the test.
my $start_lsn = $node->safe_psql('maintable',
...
my $record_end_lsn = $node->safe_psql('maintable',
...
my $end_lsn =
  $node->safe_psql('maintable', 'SELECT pg_current_wal_flush_lsn()');
