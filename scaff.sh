...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Insert a document
    let user_doc = Document::new().insert(\"name\", \"Neo\").insert(\"age\", 30);
    collection.insert_one(user_doc).await?;

    // Find the document
    let filter = Query::new().filter(\"name\", \"Neo\");
    let found_doc = collection.find_one(filter).await?;
...
create_files() {
    log "Creating core project files..."
...
create_additional_files() {
    log "Creating additional module files..."
    for file_content in "${ADDITIONAL_FILES[@]}"; do
        file=$(echo "$file_content" | cut -d ':' -f 1)
        content=$(echo "$file_content" | cut -d ':' -f 2-)
...
main() {
    ...
    log "Starting Largetable project scaffold process..."
    ...
    log "You can now navigate into the directory and begin development."
    log "Run 'cd $PROJECT_DIR' to get started."
}
