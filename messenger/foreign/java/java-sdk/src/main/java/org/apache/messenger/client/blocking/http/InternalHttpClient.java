...
import org.apache.messenger.client.blocking.http.error.MessengerHttpError;
import org.apache.messenger.client.blocking.http.error.MessengerHttpException;
...
final class InternalHttpClient {
    ...
    private void handleErrorResponse(ClassicHttpResponse response) throws IOException {
        if (!isSuccessful(response.getCode())) {
            var error = objectMapper.readValue(response.getEntity().getContent(), MessengerHttpError.class);
            throw new MessengerHttpException(error);
        }
    }
