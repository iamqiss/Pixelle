...
final class InternalTcpClient {
    ...
    private final BlockingQueue<MessengerResponse> responses = new LinkedBlockingQueue<>();
    ...
    InternalTcpClient(String host, Integer port) {
        client = TcpClient.create()
                .host(host)
                .port(port)
                .doOnConnected(conn -> conn.addHandlerLast(new MessengerResponseDecoder()));
    }

    void connect() {
        this.connection = client.connectNow();
        this.connection.inbound().receiveObject().ofType(MessengerResponse.class).subscribe(responses::add);
    }
    ...
    ByteBuf send(int command, ByteBuf payload) {
        ...
        try {
            MessengerResponse response = responses.take();
    ...
    private ByteBuf handleResponse(MessengerResponse response) {
    ...
    static class MessengerResponseDecoder extends ByteToMessageDecoder {
        @Override
        protected void decode(ChannelHandlerContext channelHandlerContext, ByteBuf byteBuf, List<Object> list) {
            ...
            list.add(new MessengerResponse(status, length, byteBuf.readBytes(length)));
        }
    }

    record MessengerResponse(long status, int length, ByteBuf payload) {
    }
