...
using Apache.Messenger.Contracts.Auth;
using Apache.Messenger.Enums;
using Apache.Messenger.Extensions;
using Apache.Messenger.Headers;
using Apache.Messenger.Messages;

namespace Apache.Messenger.Mappers;
...
internal static class BinaryMapper
{
    ...
    internal static StatsResponse MapStats(ReadOnlySpan<byte> payload)
    {
        ...
        var messengerVersionLength = BinaryPrimitives.ReadInt32LittleEndian(payload[position..(position + 4)]);
        var messengerVersion = Encoding.UTF8.GetString(payload[(position + 4)..(position + 4 + messengerVersionLength)]);
        position += 4 + messengerVersionLength;
        var messengerSemVersion = BinaryPrimitives.ReadUInt32LittleEndian(payload[position..(position + 4)]);
        position += 4;
        ...
        return new StatsResponse
        {
            ...
            MessengerServerVersion = messengerVersion,
            MessengerServerSemver = messengerSemVersion,
