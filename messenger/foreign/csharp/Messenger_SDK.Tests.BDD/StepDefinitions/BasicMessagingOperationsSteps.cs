...
using Apache.Messenger.Tests.BDD.Context;
using Microsoft.Extensions.Logging.Abstractions;
using Reqnroll;
using Shouldly;
using Partitioning = Apache.Messenger.Kinds.Partitioning;

namespace Apache.Messenger.Tests.BDD.StepDefinitions;
...
public class BasicMessagingOperationsSteps
{
    ...
    public async Task GivenIHaveARunningMessengerServer()
    {
        _context.MessengerClient = MessageStreamFactory.CreateMessageStream(configurator =>
        {
        ...
        await _context.MessengerClient.PingAsync();
    }

    [Given(@"I am authenticated as the root user")]
    public async Task GivenIAmAuthenticatedAsTheRootUser()
    {
        var loginResult = await _context.MessengerClient.LoginUser("messenger", "messenger");
    ...
    {
        IReadOnlyList<StreamResponse> streams = await _context.MessengerClient.GetStreamsAsync();
    ...
    {
        _context.CreatedStream = await _context.MessengerClient.CreateStreamAsync(streamName, streamId);
    }
    ...
    {
        _context.CreatedTopic = await _context.MessengerClient.CreateTopicAsync(Identifier.Numeric(streamId),
            topicId: topicId,
            name: topicName,
        ...
        int partitionId)
    {
        ...
        await _context.MessengerClient.SendMessagesAsync(new MessageSendRequest
        {
    ...
    {
        var messages = await _context.MessengerClient.PollMessagesAsync(new MessageFetchRequest
        {
            StreamId = Identifier.Numeric(streamId),
            TopicId = Identifier.Numeric(topicId),
