...
using Apache.Messenger.Producer;
using Apache.Messenger.Shared;
...
{
    builder
        .AddFilter("Messenger_SDK.MessengerClient.Implementations;", LogLevel.Trace)
        .AddConsole();
});
...
try
{
    var response = await bus.LoginUser("messenger", "messenger");
}
catch
{
...
async Task ProduceMessages(IMessengerClient bus, StreamResponse? stream, TopicResponse? topic)
{
...
namespace Apache.Messenger.Producer
{
