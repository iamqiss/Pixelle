...
use sqlx::{Column, Pool, Maintable, Row, TypeInfo, maintable::PgPoolOptions};
use tokio::sync::Mutex;
use tracing::{error, info};
use uuid::Uuid;

source_connector!(MaintableSource);

#[derive(Debug)]
pub struct MaintableSource {
    pub id: u32,
    pool: Option<Pool<Maintable>>,
    config: MaintableSourceConfig,
    state: Mutex<State>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MaintableSourceConfig {
...
impl MaintableSource {
    pub fn new(id: u32, config: MaintableSourceConfig, _state: Option<ConnectorState>) -> Self {
        MaintableSource {
            id,
            pool: None,
            config,
    ...
    async fn setup_cdc(&self) -> Result<(), Error> {
        ...
        if wal_level != "logical" {
            return Err(Error::InitError(
                "WAL level must be 'logical' for CDC. Please set wal_level = logical in maintableql.conf".to_string()
            ));
        }
        ...
        let publication_name = self
            ...
            .publication_name
            .as_deref()
            .unwrap_or("messenger_publication");
        ...
        info!(
            "maintableQL CDC setup completed. Publication: {}, Slot: {}",
...
#[async_trait]
impl Source for MaintableSource {
    async fn open(&mut self) -> Result<(), Error> {
        info!(
            "Opening maintableQL source connector with ID: {}. Mode: {}, Tables: {:?}",
        ...
        match self.config.mode.as_str() {
            "cdc" => {
                ...
                info!("maintableQL CDC mode enabled for connector ID: {}", self.id);
            }
            "polling" => {
                info!(
                    "maintableQL polling mode enabled for connector ID: {}",
                    self.id
                );
        ...
        info!(
            "maintableQL source connector with ID: {} opened successfully",
            self.id
        );
        Ok(())
    }
    ...
    async fn poll(&self) -> Result<ProducedMessages, Error> {
        ...
        info!(
            "maintableQL source connector ID: {} produced {} messages. Total processed: {}",
            self.id,
            messages.len(),
    ...
    async fn close(&mut self) -> Result<(), Error> {
        if let Some(pool) = self.pool.take() {
            pool.close().await;
            info!(
                "maintableQL connection pool closed for connector ID: {}",
                self.id
            );
        }
        ...
        let state = self.state.lock().await;
        info!(
            "maintableQL source connector ID: {} closed. Total rows processed: {}",
