...
use sqlx::{Pool, Maintable, maintable::PgPoolOptions};
use tokio::sync::Mutex;
use tracing::{error, info};

sink_connector!(MaintableSink);

#[derive(Debug)]
pub struct MaintableSink {
    pub id: u32,
    pool: Option<Pool<Maintable>>,
    config: MaintableSinkConfig,
    state: Mutex<State>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MaintableSinkConfig {
...
impl MaintableSink {
    pub fn new(id: u32, config: MaintableSinkConfig) -> Self {
        MaintableSink {
            id,
            pool: None,
            config,
    ...
    async fn connect(&mut self) -> Result<(), Error> {
        ...
        let pool = PgPoolOptions::new()
            ...
            .await
            .map_err(|e| Error::InitError(format!("Failed to connect to maintableQL: {e}")))?;
        ...
        self.pool = Some(pool);
        info!(
            "Connected to maintableQL database with {} max connections",
            max_connections
        );
        Ok(())
    }
    ...
    async fn ensure_table_exists(&self) -> Result<(), Error> {
        ...
        if include_metadata {
            create_table_sql.push_str(", messenger_offset BIGINT");
            create_table_sql.push_str(", messenger_timestamp TIMESTAMP WITH TIME ZONE");
            create_table_sql.push_str(", messenger_stream TEXT");
            create_table_sql.push_str(", messenger_topic TEXT");
            create_table_sql.push_str(", messenger_partition_id INTEGER");
        }

        if include_checksum {
            create_table_sql.push_str(", messenger_checksum BIGINT");
        }

        if include_origin_timestamp {
            create_table_sql.push_str(", messenger_origin_timestamp TIMESTAMP WITH TIME ZONE");
        }
    ...
    async fn process_messages(
        ...
        info!(
            "maintableQL sink ID: {} processed {} messages to table '{}'",
            self.id,
            messages.len(),
    ...
    async fn insert_batch(
        ...
        for message in messages {
            ...
            if include_metadata {
                query.push_str(
                    ", messenger_offset, messenger_timestamp, messenger_stream, messenger_topic, messenger_partition_id",
                );
                for i in 2..=6 {
            ...
            if include_checksum {
                query.push_str(", messenger_checksum");
                param_count += 1;
                values.push_str(&format!(", ${param_count}"));
            }

            if include_origin_timestamp {
                query.push_str(", messenger_origin_timestamp");
                param_count += 1;
                values.push_str(&format!(", ${param_count}"));
            }
...
#[async_trait]
impl Sink for MaintableSink {
    async fn open(&mut self) -> Result<(), Error> {
        info!(
            "Opening maintableQL sink connector with ID: {}. Target table: {}",
    ...
    async fn close(&mut self) -> Result<(), Error> {
        info!("Closing maintableQL sink connector with ID: {}", self.id);
        let state = self.state.lock().await;
        info!(
            "maintableQL sink ID: {} processed {} messages with {} errors",
