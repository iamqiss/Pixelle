...
use super::MessengerMessageHeaderViewMut;
use messenger_common::{MESSENGER_MESSAGE_HEADER_SIZE, MessengerMessageHeaderView, calculate_checksum};
use lending_iterator::prelude::*;

/// A mutable view of a message for in-place modifications
#[derive(Debug)]
pub struct MessengerMessageViewMut<'a> {
    /// The buffer containing the message
    buffer: &'a mut [u8],
}

impl<'a> MessengerMessageViewMut<'a> {
    ...
    pub fn header(&self) -> MessengerMessageHeaderView<'_> {
        let hdr_slice = &self.buffer[0..MESSENGER_MESSAGE_HEADER_SIZE];
        MessengerMessageHeaderView::new(hdr_slice)
    }

    /// Get an ephemeral mutable header view for reading/writing
    pub fn header_mut(&mut self) -> MessengerMessageHeaderViewMut<'_> {
        let hdr_slice = &mut self.buffer[0..MESSENGER_MESSAGE_HEADER_SIZE];
        MessengerMessageHeaderViewMut::new(hdr_slice)
    }
    ...
    pub fn size(&self) -> usize {
        ...
        MESSENGER_MESSAGE_HEADER_SIZE + hdr_view.payload_length() + hdr_view.user_headers_length()
    }
...
pub struct MessengerMessageViewMutIterator<'a> {
    buffer: &'a mut [u8],
    position: usize,
}

impl<'a> MessengerMessageViewMutIterator<'a> {
    pub fn new(buffer: &'a mut [u8]) -> Self {
        Self {
            buffer,
            position: 0,
...
#[gat]
impl LendingIterator for MessengerMessageViewMutIterator<'_> {
    type Item<'next> = MessengerMessageViewMut<'next>;
    ...
    fn next(&mut self) -> Option<Self::Item<'_>> {
        ...
        if self.position + MESSENGER_MESSAGE_HEADER_SIZE > self.buffer.len() {
            tracing::error!(
        ...
        let view = MessengerMessageViewMut::new(buffer_slice);
