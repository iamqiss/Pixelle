...
impl ServerCommandHandler for LoginUser {
    fn code(&self) -> u32 {
        messenger_common::LOGIN_USER_CODE
    }

    #[instrument(skip_all, name = "trace_login_user", fields(messenger_user_id = session.get_user_id(), messenger_client_id = session.client_id))]
    async fn handle(
        self,
