...
impl ServerCommandHandler for UpdatePermissions {
    fn code(&self) -> u32 {
        messenger_common::UPDATE_PERMISSIONS_CODE
    }

    #[instrument(skip_all, name = "trace_update_permissions", fields(messenger_user_id = session.get_user_id(), messenger_client_id = session.client_id))]
    async fn handle(
        self,
