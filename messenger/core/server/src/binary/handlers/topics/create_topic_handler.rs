...
impl ServerCommandHandler for CreateTopic {
    fn code(&self) -> u32 {
        messenger_common::CREATE_TOPIC_CODE
    }

    #[instrument(skip_all, name = "trace_create_topic", fields(messenger_user_id = session.get_user_id(), messenger_client_id = session.client_id, messenger_stream_id = self.stream_id.as_string()))]
    async fn handle(
        mut self,
