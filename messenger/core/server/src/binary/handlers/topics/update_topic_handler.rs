...
impl ServerCommandHandler for UpdateTopic {
    fn code(&self) -> u32 {
        messenger_common::UPDATE_TOPIC_CODE
    }

    #[instrument(skip_all, name = "trace_update_topic", fields(messenger_user_id = session.get_user_id(), messenger_client_id = session.client_id, messenger_stream_id = self.stream_id.as_string(), messenger_topic_id = self.topic_id.as_string()))]
    async fn handle(
        mut self,
