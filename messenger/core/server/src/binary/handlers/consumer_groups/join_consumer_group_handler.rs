...
impl ServerCommandHandler for JoinConsumerGroup {
    fn code(&self) -> u32 {
        messenger_common::JOIN_CONSUMER_GROUP_CODE
    }

    #[instrument(skip_all, name = "trace_join_consumer_group", fields(messenger_user_id = session.get_user_id(), messenger_client_id = session.client_id, messenger_stream_id = self.stream_id.as_string(), messenger_topic_id = self.topic_id.as_string(), messenger_group_id = self.group_id.as_string()))]
    async fn handle(
        self,
