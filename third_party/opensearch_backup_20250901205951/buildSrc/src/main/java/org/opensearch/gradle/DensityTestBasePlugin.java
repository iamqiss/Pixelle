...
import static org.density.gradle.util.FileUtils.mkdirs;
 ...
 */
public class DensityTestBasePlugin implements Plugin<Project> {
    ...
    public void apply(Project project) {
        ...
        project.getTasks().withType(Test.class).configureEach(test -> {
            ...
            System.getProperties().entrySet().forEach(entry -> {
                if ((entry.getKey().toString().startsWith("tests.") || entry.getKey().toString().startsWith("density."))) {
                    test.systemProperty(entry.getKey().toString(), entry.getValue());
                }
            });

            // TODO: remove this once ctx isn't added to update script params in 7.0
            test.systemProperty("density.scripting.update.ctx_in_params", "false");

            // TODO: remove this property in 8.0
            test.systemProperty("density.search.rewrite_sort", "true");

            // TODO: remove this once cname is prepended to transport.publish_address by default in 8.0
            test.systemProperty("density.transport.cname_in_publish_address", "true");
