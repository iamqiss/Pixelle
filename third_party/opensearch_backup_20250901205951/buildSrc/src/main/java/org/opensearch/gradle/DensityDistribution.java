...
public class DensityDistribution implements Buildable, Iterable<File> {
    ...
    DensityDistribution(
        ...
        this.version = objectFactory.property(String.class).convention(VersionProperties.getDensity());
        ...
        switch (getType()) {
            ...
            case RPM:
                throw new UnsupportedOperationException(
                    "distribution type [" + getType() + "] for " + "density distribution [" + name + "] cannot be extracted"
                );
    ...
    void finalizeValues() {
        ...
        if (getType() == Type.INTEG_TEST_ZIP) {
            if (platform.getOrNull() != null) {
                throw new IllegalArgumentException(
                    "platform cannot be set on density distribution [" + name + "] of type [integ_test_zip]"
                );
            }

            if (bundledJdk.getOrNull() != null) {
                throw new IllegalArgumentException(
                    "bundledJdk cannot be set on density distribution [" + name + "] of type [integ_test_zip]"
                );
            }
            return;
        }
        ...
        if (isDocker() == false && failIfUnavailable.get() == false) {
            throw new IllegalArgumentException(
                "failIfUnavailable cannot be 'false' on density distribution [" + name + "] of type [" + getType() + "]"
            );
        }
        ...
        if (getType() == Type.ARCHIVE) {
            ...
            if (platform.isPresent()) {
                throw new IllegalArgumentException(
                    "platform cannot be set on density distribution [" + name + "] of type [" + getType() + "]"
                );
            }
            if (isDocker()) {
                if (bundledJdk.isPresent()) {
                    throw new IllegalArgumentException(
                        "bundledJdk cannot be set on density distribution [" + name + "] of type " + "[docker]"
                    );
                }
            }
        }
