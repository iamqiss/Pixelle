...
/**
 * This class starts density.
 *
 * @density.internal
 */
class Density extends EnvironmentAwareCommand {
    ...
    // visible for testing
    Density() {
        super("Starts Density", () -> {}); // we configure logging later so we override the base class from configuring logging
        versionOption = parser.acceptsAll(Arrays.asList("V", "version"), "Prints Density version information and exits");
        daemonizeOption = parser.acceptsAll(Arrays.asList("d", "daemonize"), "Starts Density in the background")
    ...
    /**
     * Main entry point for starting density
     */
    ...
    public static void main(final String[] args) throws Exception {
        ...
        final Density density = new Density();
        int status = main(args, density, Terminal.DEFAULT);
        if (status != ExitCodes.OK) {
            ...
            if (basePath != null) {
                Terminal.DEFAULT.errorPrintln(
                    "ERROR: Density did not exit normally - check the logs at "
                        + basePath
    ...
    private static void overrideDnsCachePolicyProperties() {
        for (final String property : new String[] { "networkaddress.cache.ttl", "networkaddress.cache.negative.ttl" }) {
            final String overrideProperty = "density." + property;
    ...
    static int main(final String[] args, final Density density, final Terminal terminal) throws Exception {
        return density.main(args, terminal);
    }
