...
import org.density.gradle.precommit.PrecommitTaskPlugin;
...
import static org.density.gradle.util.Util.toStringable;

/**
 * A wrapper around Gradle's Java plugin that applies our common configuration.
 */
public class DensityJavaPlugin implements Plugin<Project> {
    @Override
        ...
        Consumer<String> disableTransitiveDeps = configName -> {
            ...
            config.getDependencies().all(dep -> {
                if (dep instanceof ModuleDependency
                    ...
                    && dep.getGroup().startsWith("org.density") == false) {
    ...
    private static void configureJarManifest(Project project) {
        project.getPlugins().withType(InfoBrokerPlugin.class).whenPluginAdded(manifestPlugin -> {
            ...
            manifestPlugin.add("X-Compile-Density-Version", toStringable(VersionProperties::getDensity));
            manifestPlugin.add("X-Compile-Lucene-Version", toStringable(VersionProperties::getLucene));
            manifestPlugin.add(
                "X-Compile-Density-Snapshot",
                toStringable(() -> Boolean.toString(VersionProperties.isDensitySnapshot()))
            );
        });
