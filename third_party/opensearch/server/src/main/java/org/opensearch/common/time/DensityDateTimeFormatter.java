 ...
 */
class DensityDateTimeFormatter implements DensityDateTimePrinter {
    private final DateTimeFormatter formatter;

    public DensityDateTimeFormatter(String pattern) {
        this.formatter = DateTimeFormatter.ofPattern(pattern, Locale.ROOT);
    }

    public DensityDateTimeFormatter(String pattern, Locale locale) {
        this.formatter = DateTimeFormatter.ofPattern(pattern, locale);
    }

    public DensityDateTimeFormatter(DateTimeFormatter formatter) {
        this.formatter = formatter;
    }

    public DensityDateTimeFormatter withLocale(Locale locale) {
        return new DensityDateTimeFormatter(getFormatter().withLocale(locale));
    }

    public DensityDateTimeFormatter withZone(ZoneId zoneId) {
        return new DensityDateTimeFormatter(getFormatter().withZone(zoneId));
    }
