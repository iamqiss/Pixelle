 ...
 * <li>cluster.name - taken from sys:density.logs.cluster_name system property because it is always set</li>
 * <li>node.name - taken from NodeNamePatternConverter, as it can be set in runtime as hostname when not set in density.yml</li>
 ...
 * </ul>
 * <p>
 * It is possible to add more or override them with <code>densitymessagefield</code>
 * <code>
 * appender.logger.layout.densitymessagefields=message,took,took_millis,total_hits,types,stats,search_type,total_shards,source,id
 * </code>
 * Each of these will be expanded into a json field with a value taken {@link DensityLogMessage} field. In the example above
 * <code>... "message":  %DensityMessageField{message}, "took": %DensityMessageField{took} ...</code>
 * the message passed to a logger will be overriden with a value from %DensityMessageField{message}
 * <p>
 * The value taken from %DensityMessageField{message} has to be a simple escaped JSON value and is populated in subclasses of
 * <code>DensityLogMessage</code>
 * <p>
 ...
 * @density.internal
 */
@Plugin(name = "DensityJsonLayout", category = Node.CATEGORY, elementType = Layout.ELEMENT_TYPE, printObject = true)
public class DensityJsonLayout extends AbstractStringLayout {

    private final PatternLayout patternLayout;

    protected DensityJsonLayout(
        String typeName,
        Charset charset,
        String[] densityMessageFields,
        int maxMessageLength,
        Configuration configuration
    ) {
        super(charset);
        this.patternLayout = PatternLayout.newBuilder()
            .withPattern(pattern(typeName, densityMessageFields, maxMessageLength))
            .withAlwaysWriteExceptions(false)
            .withConfiguration(configuration)
            .build();
    }

    private String pattern(String type, String[] densityMessageFields, int maxMessageLength) {
        ...
        map.put("cluster.name", inQuotes("${sys:density.logs.cluster_name}"));
        map.put("node.name", inQuotes("%node_name"));
        map.put("message", inQuotes("%notEmpty{%enc{%marker}{JSON} }%enc{" + messageFormat + "}{JSON}"));

        for (String key : densityMessageFields) {
            map.put(key, inQuotes("%DensityMessageField{" + key + "}"));
        }
        return createPattern(map, Stream.of(densityMessageFields).collect(Collectors.toSet()));
    }

    private String createPattern(Map<String, Object> map, Set<String> densityMessageFields) {
        StringBuilder sb = new StringBuilder();
        sb.append("{");
        String separator = "";
        for (Map.Entry<String, Object> entry : map.entrySet()) {

            if (densityMessageFields.contains(entry.getKey())) {
                sb.append("%notEmpty{");
                sb.append(separator);
    ...
    @PluginFactory
    public static DensityJsonLayout createLayout(
        String type,
        Charset charset,
        String[] densitymessagefields,
        int maxMessageLength,
        ...
        return new DensityJsonLayout(type, charset, densitymessagefields, maxMessageLength, configuration);
    }
     ...
     * @density.internal
     */
    public static class Builder<B extends DensityJsonLayout.Builder<B>> extends AbstractStringLayout.Builder<B>
        implements
            org.apache.logging.log4j.core.util.Builder<DensityJsonLayout> {
        ...
        @PluginAttribute("densitymessagefields")
        private String densityMessageFields;
        ...
        @Override
        public DensityJsonLayout build() {
            String[] split = Strings.isNullOrEmpty(densityMessageFields) ? new String[] {} : densityMessageFields.split(",");
            return DensityJsonLayout.createLayout(type, charset, split, maxMessageLength, configuration);
        }
        ...
        public String getDensityMessageFields() {
            return densityMessageFields;
        }

        public B setDensityMessageFields(String densityMessageFields) {
            this.densityMessageFields = densityMessageFields;
            return asBuilder();
        }
    ...
    public static <B extends DensityJsonLayout.Builder<B>> B newBuilder() {
        return new DensityJsonLayout.Builder<B>().asBuilder();
    }
    ...
    public String toString() {
        final StringBuilder sb = new StringBuilder("DensityJsonLayout{");
