 ...
 * @density.internal
 */
class DensityConcurrentMergeScheduler extends ConcurrentMergeScheduler {
    ...
    DensityConcurrentMergeScheduler(ShardId shardId, IndexSettings indexSettings) {
    ...
    protected MergeThread getMergeThread(MergeSource mergeSource, MergePolicy.OneMerge merge) throws IOException {
        ...
        thread.setName(
            DensityExecutors.threadName(indexSettings, "[" + shardId.getIndexName() + "][" + shardId.id() + "]: " + thread.getName())
        );
        return thread;
    }
