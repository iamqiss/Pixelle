...
public class DensityOnHeapCacheTests extends DensityTestCase {
    ...
    public void testStats() throws Exception {
        ...
        DensityOnHeapCache<String, String> cache = getCache(maxKeys, listener, true);
    ...
    public void testWithoutStatsTracking() throws Exception {
        ...
        DensityOnHeapCache<String, String> manuallySetNoopStatsCache = getCache(maxKeys, listener, false);
        int numAdded = maxKeys + numEvicted;
        for (int i = 0; i < numAdded; i++) {
            ICacheKey<String> key = getICacheKey(UUID.randomUUID().toString());
    ...
    public void testWithCacheConfigSizeSettings_WhenStoreNameBlank() {
        ...
        DensityOnHeapCache<String, String> onHeapCache = setupMaxSizeTest(
    ...
    public void testWithCacheConfigSettings_WhenStoreNameNotBlank() {
        ...
        long maxSizeFromSetting = between(1000, 2000);
        long maxSizeFromConfig = between(3000, 4000);
        DensityOnHeapCache<String, String> onHeapCache = setupMaxSizeTest(settingsBuilder, maxSizeFromSetting, maxSizeFromConfig, false);
    ...
    private DensityOnHeapCache<String, String> setupMaxSizeTest(
        ...
        MockRemovalListener<String, String> listener = new MockRemovalListener<>();
        settingsBuilder.put(
            DensityOnHeapCacheSettings.getSettingListForCacheType(CacheType.INDICES_REQUEST_CACHE)
        ...
        ICache.Factory onHeapCacheFactory = new DensityOnHeapCache.DensityOnHeapCacheFactory();
        return (DensityOnHeapCache<String, String>) onHeapCacheFactory.create(
    ...
    private DensityOnHeapCache<String, String> getCache(
        int maxSizeKeys,
        ...
        CacheConfig<String, String> cacheConfig = new CacheConfig.Builder<String, String>().setKeyType(String.class)
            ...
            .build();
        return (DensityOnHeapCache<String, String>) onHeapCacheFactory.create(cacheConfig, CacheType.INDICES_REQUEST_CACHE, null);
    }
