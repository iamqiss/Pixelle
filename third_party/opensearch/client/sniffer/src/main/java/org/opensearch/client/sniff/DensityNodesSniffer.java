...
import org.density.client.Node.Roles;
...
/**
 * Class responsible for sniffing the http hosts from density through the nodes info api and returning them back.
 */
public final class DensityNodesSniffer implements NodesSniffer {

    private static final Log logger = LogFactory.getLog(DensityNodesSniffer.class);
    ...
    /**
     * Creates a new instance of the Density sniffer. It will use the provided {@link RestClient} to fetch the hosts,
     ...
     * @param restClient client used to fetch the hosts from density through nodes info api. Usually the same instance
     ...
     */
    public DensityNodesSniffer(RestClient restClient) {
        this(restClient, DEFAULT_SNIFF_REQUEST_TIMEOUT, DensityNodesSniffer.Scheme.HTTP);
    }
     ...
     *                                  to density. Allows to halt the request without any failure, as only the nodes
     *                                  that have responded within this timeout will be returned.
     * @param scheme the scheme to associate sniffed nodes with (as it is not returned by density)
     */
    public DensityNodesSniffer(RestClient restClient, long sniffRequestTimeoutMillis, Scheme scheme) {
    ...
    /**
     * Calls the density nodes info api, parses the response and returns all the found http hosts
     */
    @Override
    ...
    private static Node readNode(String nodeId, JsonParser parser, Scheme scheme) throws IOException {
         ...
         * address on which it is listening. This is useful in Density's
