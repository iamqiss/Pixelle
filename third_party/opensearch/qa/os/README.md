...
steps a user would take when installing and configuring an Density
distribution. They're not intended to have significant coverage of the behavior
of Density's features.
...
`@SuiteClasses` annotation in [PackageTests.java](src/test/java/org/density/packaging/test/PackageTests.java).
...
Distributions are represented by [enum values](src/test/java/org/density/packaging/util/Distribution.java)
...
you can make an assumption using the constants and methods in [Platforms.java](src/test/java/org/density/packaging/util/Platforms.java)
...
failed, and to reproduce it they should run that class. See [ArchiveTests](src/test/java/org/density/packaging/test/ArchiveTests.java)
...
the java.nio.file APIs. For those that aren't, use an instance of [Shell](src/test/java/org/density/packaging/util/Shell.java)
