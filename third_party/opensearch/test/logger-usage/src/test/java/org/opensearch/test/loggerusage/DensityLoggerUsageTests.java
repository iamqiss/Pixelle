...
package org.density.test.loggerusage;
...
import org.density.common.SuppressLoggerChecks;
import org.density.common.logging.DeprecationLogger;
import org.density.test.DensityTestCase;
import org.density.test.loggerusage.DensityLoggerUsageChecker.WrongLoggerUsage;
...
public class DensityLoggerUsageTests extends DensityTestCase {
        ...
        for (Method method : getClass().getMethods()) {
            if (method.getDeclaringClass().equals(getClass())) {
                if (method.getName().startsWith("check")) {
                    ...
                    DensityLoggerUsageChecker.check(errors::add, classInputStream,
    ...
    public void testLoggerUsageCheckerCompatibilityWithLog4j2Logger() {
        for (Method method : Logger.class.getMethods()) {
            if (DensityLoggerUsageChecker.LOGGER_METHODS.contains(method.getName())) {
        ...
        for (String methodName : DensityLoggerUsageChecker.LOGGER_METHODS) {
    ...
    public void checkDeprecationLogger() {
        DeprecationLogger deprecationLogger = DeprecationLogger.getLogger(DensityLoggerUsageTests.class);
