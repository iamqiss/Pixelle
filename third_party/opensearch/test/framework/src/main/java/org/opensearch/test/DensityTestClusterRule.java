...
import org.density.test.DensityIntegTestCase.SuiteScopeTestCase;
import org.density.test.client.RandomizingClient;
import org.density.test.telemetry.tracing.StrictCheckSpanProcessor;
 ...
 * injected into {@link DensityIntegTestCase} that every integration test suite should be subclassing. In case of the parameterized test suites,
 * please subclass {@link ParameterizedStaticSettingsDensityIntegTestCase} or {@link ParameterizedDynamicSettingsDensityIntegTestCase}, depending
 * on the way cluster settings are being managed.
 */
class DensityTestClusterRule implements MethodRule {
    // Maps each TestCluster instance to the exact test suite instance that triggered its creation
    private final Map<TestCluster, DensityIntegTestCase> suites = new IdentityHashMap<>();
    ...
    private DensityIntegTestCase suiteInstance = null; // see @SuiteScope
    ...
    void beforeClass() throws Exception {
        suiteSeed = DensityTestCase.randomLong();
    }
    ...
    void afterClass() throws Exception {
        try {
            if (runTestScopeLifecycle()) {
                ...
                DensityTestCase.checkStaticState(true);
    ...
    Client clientForNode(@Nullable String node) {
        ...
        if (DensityTestCase.frequently()) {
            client = new RandomizingClient(client, DensityTestCase.random());
        }
        return client;
    }
    ...
    protected final void beforeInternal(DensityIntegTestCase target) throws Exception {
        final Scope currentClusterScope = getClusterScope(target.getClass());
        Callable<Void> setup = () -> {
            currentCluster.beforeTest(DensityTestCase.random());
        ...
        switch (currentClusterScope) {
            case SUITE:
                ...
                break;
            case TEST:
                currentCluster = buildAndPutCluster(currentClusterScope, DensityTestCase.randomLong(), target);
                setup.call();
                break;
        }
    }
    ...
    protected void before(Object target, FrameworkMethod method) throws Throwable {
        final DensityIntegTestCase instance = (DensityIntegTestCase) target;
    ...
    private Scope getClusterScope(Class<?> clazz) {
        ClusterScope annotation = DensityIntegTestCase.getAnnotation(clazz, ClusterScope.class);
        // if we are not annotated assume suite!
        return annotation == null ? Scope.SUITE : annotation.scope();
    }

    private TestCluster buildWithPrivateContext(final Scope scope, final long seed, DensityIntegTestCase target) throws Exception {
    ...
    private static boolean hasParametersChanged(
        final ParameterizedDensityIntegTestCase instance,
        final ParameterizedDensityIntegTestCase target
    ) {
    ...
    private TestCluster buildAndPutCluster(Scope currentClusterScope, long seed, DensityIntegTestCase target) throws Exception {
        ...
        synchronized (clusters) {
            ...
            switch (currentClusterScope) {
                case SUITE:
                    if (testCluster != null && target instanceof ParameterizedDensityIntegTestCase) {
                        final DensityIntegTestCase instance = suites.get(testCluster);
                        if (instance != null) {
                            assert instance instanceof ParameterizedDensityIntegTestCase;
                            if (hasParametersChanged(
                                (ParameterizedDensityIntegTestCase) instance,
                                (ParameterizedDensityIntegTestCase) target
                            )) {
    ...
    private static Class<?> getTestClass() {
        return DensityTestCase.getTestClass();
    }
    ...
    private void afterInternal(boolean afterClass, DensityIntegTestCase target) throws Exception {
        ...
        DensityIntegTestCase instance = suiteInstance;
        if (instance == null) {
            instance = target;
        }
    ...
    private void initializeSuiteScope(DensityIntegTestCase target, FrameworkMethod method) throws Exception {
        ...
        if (suiteInstance != null) {
            ...
            if (target instanceof ParameterizedDensityIntegTestCase) {
                assert suiteInstance instanceof ParameterizedDensityIntegTestCase;
                if (hasParametersChanged(
                    (ParameterizedDensityIntegTestCase) suiteInstance,
                    (ParameterizedDensityIntegTestCase) target
                )) {
